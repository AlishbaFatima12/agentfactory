...
describe('getLocaleUrl with baseUrl (subpath hosting)', () => {
    ...
    it('should use alias path from localeConfigs (ur → urdu)', () => {
        const aliasConfigs = {
            en: { label: 'English' },
            ur: { path: 'urdu', label: 'Urdu' },
        };
        const result = getLocaleUrl({
            pathname: '/agent-factory-book/docs/factory',
            currentLocale: 'en',
            targetLocale: 'ur',
            defaultLocale: 'en',
            localeConfigs: aliasConfigs,
            baseUrl: '/agent-factory-book/',
        });
        expect(result).toBe('/agent-factory-book/urdu/docs/factory');
    });

    it('should strip alias path when switching back to default', () => {
        const aliasConfigs = {
            en: { label: 'English' },
            ur: { path: 'urdu', label: 'Urdu' },
        };
        const result = getLocaleUrl({
            pathname: '/agent-factory-book/urdu/docs/factory',
            currentLocale: 'ur',
            targetLocale: 'en',
            defaultLocale: 'en',
            localeConfigs: aliasConfigs,
            baseUrl: '/agent-factory-book/urdu/',
        });
        expect(result).toBe('/agent-factory-book/docs/factory');
    });
});
