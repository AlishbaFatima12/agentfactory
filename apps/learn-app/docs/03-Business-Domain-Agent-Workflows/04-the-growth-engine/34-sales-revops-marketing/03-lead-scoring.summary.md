...
- **Three-dimension model**: Fit (does this company match our ICP?), Timing (is something happening NOW?), Engagement (do they know we exist?). Fit and Timing are equally weighted because both are deal-breakers; Engagement carries less weight because it is the dimension you can change through outreach
- **Four classification tiers**: HOT (75-100, Fit >= 25 AND Timing >= 25), WARM (55-74), CULTIVATE (35-54), NOT YET (0-34). Classification depends on both total and dimension balance
- **Miscalibrated Scoring** (agent error type #2): The model underweights a signal that domain experts recognise. A forced vendor replacement is not the same urgency as a planned evaluation, but the model treats them equally until you adjust the weights
- **Dimension breakdown as diagnostic**: Low Fit = wrong company (no action fixes it). Low Timing = right company, wrong moment (monitor). Low Engagement = they do not know you exist (marketing can fix)
- **Routing rules**: HOT goes to the best rep (not the one with capacity), WARM enters a structured nurture sequence, CULTIVATE gets quarterly review, NOT YET gets zero rep time
...
- Miscalibrated Scoring is the second agent error type, following Hallucinated Data from **L01**
