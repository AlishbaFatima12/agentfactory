...
- **6-touch, 21-day sequence structure**: Each touch adds new value (insight, case study, data point, diagnostic offer) rather than repeating the same request with increasing desperation. Touch 6 is always a graceful close with the door left open
- **Over-Automation** (agent error type #4): The agent continues executing when it should pause for human judgment. A Touch 5 with recycled personalisation tells the prospect that the person writing has run out of things to say and is letting a machine keep talking
- **Personalisation decay**: The ratio of new insight to recycled reference shifts as the sequence progresses. Touches 1-3 draw from rich research brief material; touches 4-6 work harder to find new angles and often fall short
- **Sales sequences vs marketing sequences**: Sales sequences are personalised to one person's specific situation (100% individual references); marketing sequences address shared segment experiences (0% individual references). Using a marketing sequence for a researched sales prospect signals you did not do the work
- **Exit conditions**: Reply (exit to conversation), bounce (switch channel), unsubscribe (stop immediately), silence after 6 touches (warm nurture, no re-sequence for 90 days), out-of-office (pause until return + 3 days), stakeholder forward (pause, re-research, rebuild)
- **Follow-up skill**: When a prospect responds, the sequence stops and context-aware follow-up takes over, referencing the specific conversation built across the sequence rather than starting cold
