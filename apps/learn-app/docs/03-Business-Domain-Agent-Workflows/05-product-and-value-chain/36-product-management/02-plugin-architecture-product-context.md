...
skills:
  - name: "Install and configure both PM plugins (official product-management + custom product-strategy) in Cowork"
    proficiency_level: "B1"
    category: "Technical"
    bloom_level: "Apply"
    digcomp_area: "Content-Creation"
    measurable_at_this_level: "Student successfully installs both plugins and confirms they are active by running a test command"

  - name: "Populate product.local.md with accurate product context that calibrates AI output to a specific product"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Content-Creation"
    measurable_at_this_level: "Student produces a complete product.local.md for InsightFlow with all required sections filled, and verifies that a test command output reflects the product context"

learning_objectives:
  - objective: "Map each plugin command to its source plugin (official vs custom) and explain what workflow gap each plugin addresses"
    proficiency_level: "B1"
    bloom_level: "Understand"
    assessment_method: "Student can fill in a command-to-plugin table without reference to the lesson"

  - objective: "Configure product.local.md for InsightFlow by completing all five core sections: Product Identity, Vision, Personas, Engineering Team, and Stakeholder Map"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student produces a complete product.local.md and runs a test command whose output demonstrably reflects the InsightFlow context"

  - objective: "Verify plugin installation by running a brief test command and evaluating whether the output reflects the product context"
    proficiency_level: "B1"
    bloom_level: "Evaluate"
    assessment_method: "Student invokes /brief with a simple InsightFlow prompt and confirms the output names InsightFlow, Analyst Alex, and at least one current roadmap theme"

cognitive_load:
  new_concepts: 4
  concepts_list:
    - "Two-layer plugin architecture (official Layer 1 + custom Layer 2)"
...
differentiation:
  ...
  remedial_for_struggling: "Focus on just the Product Identity and Personas sections first. Run the test command after completing those two sections, before filling in Engineering Team and Stakeholder Map. Does the output already sound more specific? Add the remaining sections one at a time and see what each one adds to the output quality."
