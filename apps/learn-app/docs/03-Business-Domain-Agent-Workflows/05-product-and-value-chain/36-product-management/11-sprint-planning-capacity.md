...
skills:
  - name: "Plan a sprint by scoping the prioritised backlog against real team capacity using /sprint-planning"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can invoke /sprint-planning with capacity constraints and a backlog, receive a sprint plan, and verify that planned load is within 70-80% of available capacity"

  - name: "Evaluate a sprint plan for focus, risk coverage, and stretch item identification"
    proficiency_level: "B2"
    category: "Applied"
    bloom_level: "Evaluate"
    digcomp_area: "Information-Literacy"
    measurable_at_this_level: "Student can assess whether the sprint goal is singular and actionable, whether risks and dependencies are flagged, and whether stretch items are clearly identified for potential cuts"

learning_objectives:
  - objective: "Calculate real team capacity for a 2-week sprint by subtracting PTO, on-call rotations, and meeting overhead from raw available days"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student builds a capacity table with at least 3 team members, each with individual deductions, arriving at a team total that is then reduced to 70-80% planning capacity"

  - objective: "Produce a sprint plan using /sprint-planning that includes a single sprint goal, P0/P1/P2 prioritised backlog, risk table, and definition of done"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student invokes /sprint-planning for InsightFlow Sprint 1 using carryover stories and a partially constrained team, and receives a complete sprint plan document"

  - objective: "Evaluate the sprint plan's load against capacity and identify which items to cut or move to stretch if the load exceeds 80%"
    proficiency_level: "B2"
    bloom_level: "Evaluate"
    assessment_method: "Student reviews the sprint plan's capacity percentage and can articulate which P1 or P2 items would be the first to cut if the sprint runs over"

cognitive_load:
  new_concepts: 4
  concepts_list:
    - "Capacity planning: subtracting overhead from raw availability to reach planning capacity"
...
1. **Cut WF-006 (dashboard load, 3 pts)** → brings load to 26/30 = 87%. Better, but the tech debt was deliberately prioritised in L10. Document the trade-off.
2. **Move WF-005 (error handling, 5 pts) to stretch** → brings P0-only load to 21/30 = 70%. Clean, but error handling is needed for Sprint 2 testing. Risk is you create a P0 blocker in Sprint 2.
3. **Accept 97% and flag it** → only defensible if Leo is back from PTO before WF-005 is scoped. Check the dependency chain.
...
Continue to [Lesson 12: Stakeholder Communication →](./12-stakeholder-communication.md)
