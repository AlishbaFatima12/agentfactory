...
options: ["The vendor blind spot failure", "The static optimisation trap", "The reconciliation swamp failure", "The compliance monitoring gap"],
correctOption: 2,
...
options: ["Invoices are matched manually rather than automatically", "Suppliers are monitored only at contract renewal rather than continuously", "Network design uses annual snapshots rather than real-time data", "Freight carriers are scored on cost alone, ignoring service levels"],
correctOption: 1,
...
options: ["Reconciliation swamp", "Vendor blind spot", "Compliance monitoring gap", "Static optimisation trap"],
correctOption: 3,
...
options: ["The skills are too complex to route automatically", "Procurement tasks require human selection to maintain audit trails", "Each skill is invoked directly, making a routing layer unnecessary", "The plugin was designed for single-user rather than team use"],
correctOption: 2,
...
options: ["/kraljic-map, /three-way-match, /spend-analytics", "/invoice-reconcile, /vendor-communicate, /supply-network-design", "/carrier-scorecard, /risk-monitor, /exit-protocol", "/spend-classify, /contract-draft, /supplier-onboard"],
correctOption: 1,
...
options: ["Tactical", "Bottleneck", "Commodity", "Strategic"],
correctOption: 3,
...
options: ["Low-spend items always receive less management attention", "Bottleneck items combine low spend with high supply dependency, creating hidden vulnerability", "Strategic items are more dangerous because they carry both high spend and high risk", "Commodity items are riskiest because they are purchased from many vendors simultaneously"],
correctOption: 1,
...
options: ["Strategic", "Bottleneck", "Tactical", "Commodity"],
correctOption: 2,
...
options: ["Greater than 80%", "Greater than 85%", "Greater than 90%", "Greater than 95%"],
correctOption: 2,
...
options: ["Operational", "Compliance", "Strategic", "Geopolitical"],
correctOption: 3,
...
options: ["Less than 0.5%", "Less than 1.0%", "Less than 1.5%", "Less than 2.0%"],
correctOption: 2,
...
options: ["Invoice, contract, and delivery note", "Invoice, purchase order, and goods receipt", "Invoice, bank statement, and supplier agreement", "Purchase order, payment run, and audit log"],
correctOption: 1,
...
options: ["When the invoice exceeds the materiality threshold", "For services, utilities, or items below the materiality threshold", "For all Strategic quadrant suppliers to speed up payment", "When the supplier has an OTD score above 90%"],
correctOption: 1,
...
options: ["As a low-priority formatting error requiring supplier confirmation", "As a medium-priority discrepancy to be resolved in the next batch", "As a high-priority fraud flag requiring immediate investigation", "As a routine update to be applied automatically after 48 hours"],
correctOption: 2,
...
options: ["As three separate isolated exceptions requiring individual resolution", "As a systematic issue requiring root-cause investigation at the supplier", "As a data entry error on the buyer's side requiring internal correction", "As a tolerance breach that can be waived if the amounts are small"],
correctOption: 1,
...
options: ["LOW, because absence of negative data implies low risk", "MEDIUM, as a neutral starting position pending further review", "UNASSESSED, because a rating cannot be justified without data", "HIGH, to apply the precautionary principle until data is gathered"],
correctOption: 2,
...
options: ["Commodity", "Tactical", "Bottleneck", "Strategic"],
correctOption: 3,
...
options: ["As an acceptable level of supply chain agility", "As a signal of an upstream planning or supplier performance problem", "As a cost that should be absorbed in the logistics budget", "As a carrier reliability issue requiring scorecard review only"],
correctOption: 1,
...
options: ["Invoice discrepancy rate >15% and supplier OTD <90%", "Demand change >20% and carrier cost increase >15%", "Spend concentration >60% and geopolitical risk elevated", "Bottleneck supplier count >3 and Strategic supplier count >5"],
correctOption: 1,
...
options: ["A Gantt chart showing implementation milestones", "A sensitivity analysis testing key assumptions", "A full list of all current carrier contracts", "A carbon emission reduction forecast for each scenario"],
correctOption: 1,
...
options: ["Greater than 5%", "Greater than 8%", "Greater than 10%", "Greater than 15%"],
correctOption: 1,
...
options: ["Captured → Active → Identified", "Identified → Active → Captured", "Active → Identified → Captured", "Identified → Captured → Active"],
correctOption: 1,
...
options: ["Dispute communication", "Corrective Action Request (CAR)", "Non-renewal notice", "Emergency assurance communication"],
correctOption: 1,
...
options: ["It satisfies internal KPI reporting requirements", "It provides legal and contractual evidence in disputes and exit processes", "It enables the AI agent to learn from past communications", "It reduces the time required to draft future communications"],
correctOption: 1,
...
options: ["Issue the non-renewal notice to the supplier", "Initiate the supplier performance review and renewal decision", "Launch the emergency RFQ for replacement sourcing", "Send the corrective action request for outstanding issues"],
correctOption: 1,
...
options: ["The invoice-monitor agent", "The risk-sentinel agent", "The procurement-calendar agent", "The spend-tracker agent"],
correctOption: 2,
...
options: ["To calculate the final invoice settlement amount owed to the exiting vendor", "To identify what went wrong, update the supplier risk framework, and prevent recurrence", "To formally close the supplier record in the ERP system", "To notify remaining suppliers about the change in the supply base"],
correctOption: 1,
...
options: ["Only after the replacement supplier has been identified internally", "Immediately, in parallel with serving the exit notice, to protect supply continuity", "After the post-mortem has been completed and lessons documented", "At the 30-day mark once the exiting vendor has confirmed wind-down plans"],
correctOption: 1,
...
options: ["That individual skills are more powerful when used in isolation for focused tasks", "That full supply chain intelligence requires orchestrating skills and agents as a system", "That the plugin is too complex for everyday procurement use", "That agents should replace human procurement professionals entirely"],
correctOption: 1,
...
options: ["To introduce new skills not covered in the main lessons", "To provide a consolidated reference for skill invocations, thresholds, and agent triggers", "To summarise the academic theory behind supply chain management", "To list all plugin installation commands for technical setup"],
correctOption: 1,
...
options: ["The reconciliation swamp only", "The vendor blind spot and the static optimisation trap", "The compliance monitoring gap only", "All three structural failures simultaneously"],
correctOption: 1,
...
options: ["Strategic, due to the high spend", "Tactical, due to high spend and low supply risk", "Bottleneck, due to the number of available vendors", "Commodity, due to low spend and low risk"],
correctOption: 1,
...
options: ["HIGH financial risk, because undisclosed financials are inherently risky", "LOW financial risk, because operational performance suggests financial stability", "UNASSESSED, because a rating cannot be made without the required data", "MEDIUM financial risk, as a neutral default pending document request"],
correctOption: 2,
...
options: ["Three-way match using the most recent purchase order as a proxy GR", "Two-way match comparing invoice against the purchase order only", "Four-way match adding the signed statement of work", "Hold payment until a retrospective goods receipt is created"],
correctOption: 1,
...
options: ["Stage 1: Document ingestion and field extraction", "Stage 2: Tolerance checking against the purchase order", "Stage 3: Pattern detection across the exception log", "Stage 4: Escalation routing and approval workflow"],
correctOption: 2,
...
options: ["No action, as 88% is within an acceptable operational range", "A corrective action request, as OTD is below the 90% threshold", "Immediate non-renewal notice due to sustained underperformance", "A Kraljic reclassification from Strategic to Bottleneck"],
correctOption: 1,
...
options: ["Wait for the next scheduled quarterly review to assess impact", "Automatically place the supplier order on hold pending human approval", "Raise an alert with a preliminary risk assessment for human review", "Reclassify the supplier to Bottleneck status in the risk register"],
correctOption: 2,
...
options: ["Excellent carrier performance with minor planning variability", "An upstream planning or supplier failure masking carrier performance data", "A carrier that specialises in urgent freight and should be used more widely", "A data quality issue where expedited and standard bookings are miscategorised"],
correctOption: 1,
...
options: ["Board-level sign-off before the analysis is shared with operations", "A sensitivity analysis showing how the recommendation changes under different cost assumptions", "A complete list of all current carrier contracts and termination clauses", "An environmental impact statement quantifying the Scope 3 reduction"],
correctOption: 1,
...
options: ["Diversify to reduce single-vendor dependency immediately", "Assess whether concentration provides leverage for renegotiation", "Split spend across more vendors to reduce Bottleneck risk", "Flag for geopolitical risk review given high supply concentration"],
correctOption: 1,
...
options: ["Identified, because no contract is signed yet", "Active, because the initiative is underway with vendors engaged", "Captured, because the preferred vendor has been selected", "Deferred, because final decision is pending budget approval"],
correctOption: 1,
...
options: ["Dispute communication", "Corrective Action Request", "Non-renewal notice", "Emergency assurance communication"],
correctOption: 2,
...
options: ["To satisfy the procurement-calendar agent's data requirements", "To provide evidence in potential disputes, contract terminations, or regulatory investigations", "To ensure the risk-sentinel agent can access communication history for scoring", "To demonstrate to internal stakeholders that the procurement team is active"],
correctOption: 1,
...
options: ["To initiate the formal supplier performance review", "To execute final transition steps and confirm handover readiness", "To issue the non-renewal or renewal notice to the supplier", "To launch the emergency RFQ process for replacement sourcing"],
correctOption: 1,
...
options: ["Procurement-calendar agent", "Invoice-monitor agent", "Risk-sentinel agent", "Spend-tracker agent"],
correctOption: 2,
...
options: ["A non-renewal notice confirming the exit timeline", "A dispute communication citing the contractual documentation obligation", "An emergency assurance communication requesting immediate delivery confirmation", "A corrective action request for the documentation failure"],
correctOption: 1,
...
options: ["60 days is the minimum notice period required by UK procurement regulations", "60 days represents the tight window that requires concurrent action on multiple tracks", "60 days is the standard contract notice period in most commercial agreements", "60 days is chosen because it matches the procurement-calendar cascade intervals"],
correctOption: 1,
...
options: ["Regulatory compliance requires Kraljic classification before any supplier contact", "Kraljic classification determines the management strategy for all subsequent phases", "The invoice-reconcile skill cannot function without a prior Kraljic assessment", "Phase 1 must be completed before the plugin can be installed"],
correctOption: 1,
...
options: ["The reconciliation swamp, because invoice-monitor is not active", "The vendor blind spot, because continuous monitoring requires agents not just skills", "The static optimisation trap, because the network design skill alone is insufficient", "All three structural failures, because skills without agents provide no intelligence"],
correctOption: 1,
...
options: ["Eight is the maximum number of skills a Claude plugin can contain", "Each skill maps to a distinct analytical domain with different data inputs and outputs", "Fewer skills would reduce the plugin's marketability in the agent marketplace", "Eight skills are required to cover the seven capstone phases plus one overlap"],
correctOption: 1,
...
options: ["Commercial and financial dimensions", "Operational and compliance dimensions", "Geopolitical and strategic dimensions", "Geopolitical and compliance dimensions"],
correctOption: 3,
...
options: ["Yes, if the 5% quantity variance is within the configured tolerance band", "No, because the high invoice value requires manual approval regardless of variance", "Yes, because quantity variances under 10% are always within tolerance", "No, because three-way match quantity variances always require manual resolution"],
correctOption: 0,
...
options: ["No action, because the Tier 1 supplier's scores remain strong", "Initiate a Six-Dimension reassessment of the Tier 1 supplier only", "Activate contingency sourcing review because Tier 2 failure threatens Tier 1 continuity", "Reclassify the Tier 1 supplier from Strategic to Bottleneck immediately"],
correctOption: 2,
...
options: ["Alphabetical order, for process consistency", "By Kraljic classification: Strategic first, then Bottleneck, then Tactical", "By absolute spend value, largest category first", "By contract expiry date, soonest expiry first"],
correctOption: 1,
...
options: ["Agents are cheaper to run than repeated skill invocations", "Agents provide persistent, continuous monitoring that on-demand skill invocations cannot", "Skills cannot process the volume of data required for enterprise procurement", "Agents are required by the plugin specification format even if skills could do the work"],
correctOption: 1,
