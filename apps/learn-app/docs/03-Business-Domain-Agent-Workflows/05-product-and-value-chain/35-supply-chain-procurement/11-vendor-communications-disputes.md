...
keywords:
  [
    ...
    "CAR",
    "non-renewal notice",
    "emergency supply assurance",
    "exit notice",
    "vendor-communicate skill",
    "supplier dispute",
...
skills:
  - name: "Draft Vendor Communications Using Professional Procurement Standards"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Communication"
    measurable_at_this_level: "Student can identify the correct communication type for a given scenario, draft a compliant communication using /vendor-communicate, and verify that the output contains the required elements (PO reference, specific data, required action, deadline)"

  - name: "Apply Tone Calibration and Authority Levels to Vendor Communications"
    proficiency_level: "B1"
    category: "Soft"
    bloom_level: "Evaluate"
    digcomp_area: "Communication"
    measurable_at_this_level: "Student can distinguish between the five communication types, explain when each is appropriate, and identify which authority level is required to send each type"

learning_objectives:
  - objective: "Identify the appropriate communication type (dispute notice, CAR, non-renewal, emergency assurance, exit notice) for a given procurement scenario"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Given five procurement scenarios, student selects the correct communication type and explains the selection based on trigger conditions defined in the lesson"

  - objective: "Draft a legally defensible invoice dispute notice using /vendor-communicate with specific PO references, variance data, and required action"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student invokes /vendor-communicate for a dispute scenario and verifies the output contains: invoice number, PO reference, specific variance description, required action (credit note or revised invoice), and deadline"

  - objective: "Explain the authority requirements for each communication type and why communications above threshold require escalated sign-off"
    proficiency_level: "B1"
    bloom_level: "Understand"
    assessment_method: "Student maps each of the five communication types to its required authority level and articulates why the escalation threshold exists (legal exposure, relationship management, strategic impact)"
...
- PO reference number
- Specific discrepancy with figures (invoiced vs. our records vs. variance amount)
- Contractual or PO basis for the disputed amount
- Required action (credit note / revised invoice / evidence)
- Deadline (typically 5 business days)
- Confirmation that undisputed amount will be processed on standard terms

**Authority level:** Category Manager sign-off
...
- The four items the plan must address: root cause, containment actions already taken, corrective actions with target dates, monitoring approach
- Proposed review meeting date
- Contractual clause under which the notice is issued

**Authority level:** CPO sign-off
...
**Required elements:**
- Contract reference and agreement date
- Contractual notice period and notice deadline date (must confirm sent before this date)
...
**Required elements:**
- Brief, factual description of the triggering signal (no speculation, no accusation)
- Specific questions about production capacity, raw material inventory, and confirmed order fulfillment
- Response deadline (3-5 business days)
- Reference to the possibility of a follow-up call with senior management

**Authority level:** Category Manager or CPO, depending on vendor tier
...
```
/vendor-communicate type:"dispute-notice"
                    vendor:"AlphaSteel Corp"
                    contact:"James Holbrook, Finance Director"
                    invoice:"INV-2024-1147"
                    invoice-date:"2026-03-10"
                    invoice-amount:"£48,250"
                    po-reference:"PO-2024-0892"
                    po-amount:"£46,800"
                    goods-receipt:"GR-2024-1103 (confirmed delivery matches PO)"
                    discrepancy:"£1,450 overcharge — no contractual basis for variance"
                    required-action:"Credit note for £1,450"
                    deadline:"2026-03-20"
                    undisputed-amount:"£46,800"
```
...
| Element | Check |
| --- | --- |
| Invoice number and date | Correctly referenced |
| PO reference | Included as the contractual basis |
| Discrepancy breakdown | Invoiced (£48,250) vs. PO (£46,800) vs. variance (£1,450) |
| Basis for dispute | GR confirmation that delivery matched PO |
| Required action | Credit note for specific amount |
| Deadline | Specific date, 5 business days |
| Undisputed payment | Confirmed for processing on standard terms |

:::note Verify the data, not the phrasing
...
```
/vendor-communicate type:"corrective-action-request"
                    vendor:"PrecisionCast Ltd"
                    contact:"Sarah Chen, Operations Director"
                    car-reference:"CAR-2026-004"
                    kpi:"On-time delivery"
                    contractual-sla:"92%"
                    actual-performance:"71% (8-week average)"
                    shortfall:"21 percentage points"
                    period:"2026-01-13 to 2026-03-10"
                    data-reference:"ERP goods receipt report GR-RPT-2026-Q1-W10"
                    plan-deadline:"14 calendar days"
                    review-date:"2026-04-01"
                    contract-clause:"Clause 8.3 of Supply Agreement dated 2024-06-01"
```

The output should produce a formal CAR document with the CAR reference number, performance data table, four-point plan requirements, and review meeting invitation. This document should be reviewed by the CPO before sending.

## Communication Standards: What to Verify in Every Output

Regardless of communication type, verify these standards in every `/vendor-communicate` output:
...
| Wrong | Right |
| --- | --- |
| "We are disappointed with your continued failure to meet delivery targets" | "OTD against the 92% contractual SLA averaged 71% during the 8-week period to 10 March 2026" |
| "Your invoice is clearly wrong and we refuse to pay it" | "Invoice INV-2024-1147 contains a discrepancy of £1,450 not supported by PO-2024-0892 or goods receipt GR-2024-1103" |
| "We need this fixed immediately" | "Please provide a credit note for £1,450 by 20 March 2026" |
| "This is unacceptable and reflects poorly on your organisation" | "This notice is issued under Clause 8.3 of our Supply Agreement dated 1 June 2024" |

The left column gives the vendor an opening to dispute your process or tone rather than your substance. The right column leaves them only one option: address the specific data.
...
```
Draft an invoice dispute notice for the following scenario:

Vendor: GlobalParts Ltd
Invoice: INV-2026-0551, dated 12 March 2026, amount £23,400
PO reference: PO-2026-0218, amount £22,000
Goods receipt: GR-2026-0412 — delivery confirmed 80 units as per PO
Discrepancy: £1,400 — vendor has invoiced 87 units; goods receipt confirms 80 units
Required action: Credit note for £1,400 (7 units × £200/unit)
Deadline: 5 business days from today
Undisputed payment: £22,000 to be processed on standard 30-day terms
...
```
Think of an invoice discrepancy from your own organisation — or a
realistic one for your industry. Describe:

- The invoice amount and what was invoiced
- The PO amount and what was agreed
- The goods receipt confirmation (what was actually received/delivered)
- The discrepancy type: price variance, quantity variance, or unauthorised charge
- The required resolution

Draft an invoice dispute notice for this scenario. Then evaluate:
Does the output contain a specific PO reference? A specific variance
amount with basis? A clear required action with a deadline?
Adjust the prompt until the output meets all five standards.
```

**What you are learning:** Translating a real discrepancy into a structured dispute notice requires you to separate the facts from the frustration. The prompt structure forces that discipline.

**Apply**: Extend to a new situation the lesson didn't cover directly.

```
Your Vendor Health Monitor has flagged that one of your Strategic
suppliers (annual spend £2.4M, sole-source for a critical component)
has been mentioned in a financial news article reporting that their
parent company is under review for a potential sale.

No confirmation of supply impact yet. But this vendor has no backup
supplier and a 90-day qualification timeline for any alternative.

Draft the appropriate vendor communication for this scenario.
Then answer: What type of communication is this? What authority level
is required before sending? What specific written assurance are you
seeking?
```
...
Continue to [Lesson 12: Persistent Agents & Schedule →](./12-persistent-agents-schedule.md)
