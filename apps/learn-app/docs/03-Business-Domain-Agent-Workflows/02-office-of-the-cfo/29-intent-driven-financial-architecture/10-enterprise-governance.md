...
skills:
  ...
  - name: "Model Registry Architecture"
    ...
    measurable_at_this_level: "Student can create a Model Registry template with all required fields (model name, owner, validation date, Intent Note coverage %, model link) and design a quarterly review process"
...
learning_objectives:
  ...
  - objective: "Create a Model Registry template with all required fields and design a quarterly review workflow that tracks Intent Note coverage across the organisation"
    proficiency_level: "B2"
    bloom_level: "Create"
    assessment_method: "Student produces a Model Registry spreadsheet template with all five required fields and a review checklist; the template is usable without modification"

  - objective: "Apply the four governance artefacts to a team scenario, specifying which artefact addresses each governance gap and explaining how each artefact complements the plugin's technical enforcement"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Given a description of a team with governance problems, student correctly maps each problem to the artefact that solves it and explains why the plugin alone cannot solve it"

cognitive_load:
  new_concepts: 6
  concepts_list:
    - "Plugin enforcement vs organisational governance (the two layers)"
    - "Standards Document addendum (what the plugin doesn't cover)"
    - "Model Registry (centralised model tracking)"
