...
skills:
  - name: "Financial Scenario Testing with AI"
    proficiency_level: "A2"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Problem Solving"
    measurable_at_this_level: "Student can describe a multi-variable scenario to Claude in Excel, interpret the downstream impacts on key financial outputs, and explain why combined effects differ from the sum of individual changes"

  - name: "Formula Error Diagnosis with AI"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Analyze"
    digcomp_area: "Problem Solving"
    measurable_at_this_level: "Student can present a formula error to Claude, follow the traced dependency chain from symptom to source, and apply the recommended fix while predicting whether secondary errors will resolve"

  - name: "Financial Model Construction from Description"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Content Creation"
    measurable_at_this_level: "Student can write a structured plain-language description specifying tabs, line items, and linkage requirements, commission a model through Claude in Excel, and validate the resulting formula architecture against the specification"

learning_objectives:
  - objective: "Apply Claude in Excel to test multi-variable financial scenarios, interpret compounding effects on downstream outputs, and revert to the base case without formula damage"
    proficiency_level: "A2"
    bloom_level: "Apply"
    assessment_method: "Student runs a multi-input scenario, documents the combined impact on free cash flow, and explains why the combined effect differs from the sum of individual changes"

  - objective: "Analyse formula error chains by presenting symptoms to Claude, following the traced dependency path from output cell to root cause, and applying fixes that resolve both primary and secondary errors"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Given a workbook with a #REF! or text-number error, student traces the chain with Claude, predicts secondary effects, applies the fix, and confirms resolution"

  - objective: "Construct a two-year three-statement financial model by writing a structured plain-language specification, commissioning the structure through Claude in Excel, and validating the formula linkages across tabs"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student writes a model specification, commissions the build, and verifies that changing one revenue assumption flows correctly through P&L, cash flow, and balance sheet"

cognitive_load:
  new_concepts: 7
  concepts_list:
    - "Sensitivity analysis (one-way and two-way tables)"
    - "DCF fundamentals (free cash flow, discount rate, terminal value)"
    - "Common Excel formula errors (#REF!, #VALUE!, #DIV/0!, #N/A, circular references)"
    - "Error tracing from symptom to source"
    - "Text-number formatting mismatch"
    - "EBITDA definition and its role in valuation"
    - "Three financial statements and their linkages"
  assessment: "7 concepts across three skills at A2-B1. Each concept is introduced inside a dedicated concept box before it is applied, keeping working memory demands within the B1 ceiling of 7-10. Students already understand Claude in Excel's general capabilities from Lesson 2."

differentiation:
  extension_for_advanced: "After completing Exercise 4, extend the three-statement model with a balance sheet tab that includes fixed assets (driven by capex and depreciation), trade debtors (driven by debtor days), and a balancing cash line. Verify that Total Assets equals Total Liabilities plus Equity after any assumption change."
  remedial_for_struggling: "Focus on Exercise 3 (Error Diagnosis). Create a simple SUM formula across ten cells, then deliberately format three of them as text by prefixing with an apostrophe. Ask Claude to diagnose the discrepancy. If you can follow the explanation and fix the issue, you understand the core error-tracing pattern this lesson teaches."
...
teaching_guide:
  ...
  assessment_checks:
    - question: "What is the difference between a one-way and a two-way sensitivity table?"
      expected_response: "A one-way table varies one input across a range to show its impact on an output. A two-way table varies two inputs simultaneously, producing a grid that shows the combined impact of every combination."
    - question: "Why does SUM ignore text-formatted numbers, and how do you detect them?"
      expected_response: "SUM treats text-formatted cells as non-numeric and skips them. The visual indicator is left-alignment (numbers right-align by default). The fix is Data → Text to Columns → Finish, which forces Excel to re-evaluate them as numbers."
    - question: "What three elements should a model-building specification include to produce a usable starting point?"
      expected_response: "Named tabs (Assumptions, P&L, Cash Flow), explicit line items for each tab, and stated linkage requirements (e.g., depreciation on P&L references fixed assets on Assumptions, cash flow starts with net income from P&L)."
---
...
>
> Sensitivity analysis tests how much an output (like company value) changes when you vary one or more inputs (like growth rate). It answers: _which assumptions matter most?_
>
