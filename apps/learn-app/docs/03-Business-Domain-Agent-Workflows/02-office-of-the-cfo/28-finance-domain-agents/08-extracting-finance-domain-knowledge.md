...
skills:
  - name: "Apply Knowledge Extraction to Finance Domain"
  ...
  - name: "Author a Finance Domain SKILL.md"
    ...
    measurable_at_this_level: "Student can translate extraction outputs from a finance expert interview into a first-draft SKILL.md with a functional Persona section, a Questions section with in-scope and out-of-scope boundaries, and testable Principles grounded in firm-specific heuristics"

  - name: "Distinguish Firm-Specific from Generic Finance Knowledge"
    proficiency_level: "B1"
    category: "Conceptual"
    bloom_level: "Analyze"
...
learning_objectives:
  ...
  - objective: "Translate finance extraction outputs into a first-draft SKILL.md with a functional Persona, scoped Questions section, and testable Principles that encode firm-specific heuristics"
    proficiency_level: "B2"
    bloom_level: "Apply"
    assessment_method: "Given a set of interview notes from a CFO, student can produce a SKILL.md draft whose Persona addresses the three writing questions, whose Questions section includes out-of-scope boundaries, and whose Principles pass the testability criterion"

  - objective: "Identify which elements of a finance workflow represent firm-specific tacit knowledge and explain why encoding those elements produces value that generic plugins cannot replicate"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Student can examine the monthly close workflow and distinguish firm-specific elements (materiality thresholds, seasonal patterns, bridge format preferences) from generic elements (GAAP rules, standard variance calculations), and explain the value gap"

cognitive_load:
  new_concepts: 4
  concepts_list:
    - "The Five Questions adapted to finance (from generic extraction to domain-specific extraction)"
    - "Firm-specific vs generic knowledge in finance workflows (the gap that SKILL.md fills)"
    - "SKILL.md authoring from finance interview outputs (Persona, Questions, Principles for a close workflow)"
    - "Document extraction applied to close checklists and accounting policy manuals (Method B in finance)"
  assessment: "4 concepts at B1-B2 level — within the 7-10 cognitive limit for this tier. Students have already internalised the Five Questions (Ch16 L2), the interview technique (Ch16 L3), and the SKILL.md writing process (Ch16 L6). This lesson applies that methodology to a concrete finance domain, which reduces novelty because the framework is familiar."

differentiation:
  extension_for_advanced: "Conduct a self-interview using the Five Questions applied to your own finance domain. Record your answers and assess them: for each answer, is the knowledge firm-specific or generic? Calculate the ratio. Then draft a north star summary and three testable Principles from your own extraction material."
  remedial_for_struggling: "Focus on the generic-vs-specific distinction in the variance analysis example. Take the generic statement 'We investigate material variances.' Rewrite it with a specific dollar threshold, a specific account type, and a specific action. If you can make that transformation, you understand the core extraction skill this lesson applies."

teaching_guide:
  lesson_type: "core"
  session_group: 3
  session_title: "Knowledge Extraction Applied"
  key_points:
    - "Generic finance plugins use industry-standard thresholds — firm-specific SKILL.md files encode the CFO's actual materiality logic, seasonal patterns, and bridge format preferences"
    - "Each of the Five Questions surfaces a different layer of finance tacit knowledge: undocumented heuristics, override conditions, leading indicators, experience gaps, and day-one instructions"
    - "The worked example shows the full pipeline: CFO interview → extraction notes → north star summary → SKILL.md draft with Persona, Questions, and Principles"
    - "Method B applied to close checklists and accounting manuals surfaces explicit rules, contradictions between policy and practice, and gaps where no policy exists"
  misconceptions:
    - "Students may think the generic finance plugin is sufficient — the plugin handles standard workflows, but the value gap is filled by the CFO's judgment about which variances matter at this specific firm"
    - "Students may confuse GAAP rules with tacit knowledge — GAAP is codified and generic; the CFO's interpretation of how GAAP applies to this firm's specific business model is tacit"
    - "Students may write Principles that restate accounting standards rather than encoding firm-specific heuristics — the testability check prevents this"
  discussion_prompts:
    - "Think about your organisation's monthly close. What does an experienced controller know that a new hire does not? That gap is exactly what the extraction targets."
...
| Variance                                       | Generic Plugin Response              | CFO's Actual Response                                                                                           |
| ---------------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| Revenue down $50K against a $20M budget        | Flags for review (exceeds threshold) | Ignores — within normal monthly noise for this business                                                         |
| Travel expense up $5K in a dormant cost centre | Ignores — below threshold            | Investigates immediately — dormant accounts with activity are a potential fraud signal                          |
