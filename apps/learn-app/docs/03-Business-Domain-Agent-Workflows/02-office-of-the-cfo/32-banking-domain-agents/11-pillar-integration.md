...
keywords:
  [
    ...
    "IFRS 9 Basel interaction",
    "AML capital impact",
    "ECL CET1 impact",
    "provision capital linkage",
    "banking agent skill chaining",
    "regulatory cascade",
    "IRB shortfall",
...
skills:
  - name: "Trace a Cross-Pillar Regulatory Cascade"
    proficiency_level: "B2"
    category: "Applied"
    bloom_level: "Analyze"
    digcomp_area: "Data Literacy"
    measurable_at_this_level: "Student can trace an event from its origin in one pillar through its impacts in the other two pillars, identifying each causal link and quantifying the downstream effect on capital ratios"

  - name: "Calculate CET1 Impact of an IFRS 9 Provision Increase"
    proficiency_level: "B1"
    category: "Technical"
    bloom_level: "Apply"
    digcomp_area: "Digital Content Creation"
    measurable_at_this_level: "Student can calculate the post-tax CET1 impact of a provision charge, compute the revised CET1 ratio, and determine whether the bank breaches its combined buffer requirement"

  - name: "Evaluate Pillar-Isolated vs Integrated Agent Architectures"
    proficiency_level: "B2"
    category: "Conceptual"
    bloom_level: "Evaluate"
    digcomp_area: "Problem Solving"
    measurable_at_this_level: "Student can explain why separate IFRS 9, Basel, and AML agents that do not communicate produce incomplete regulatory advice, citing specific cross-pillar impacts that would be missed"

learning_objectives:
  - objective: "Trace a cross-pillar cascade from AML trigger through IFRS 9 staging to Basel capital impact"
    proficiency_level: "B2"
    bloom_level: "Analyze"
    assessment_method: "Student traces the fraud cascade scenario step-by-step, identifying each pillar transition and quantifying ECL and capital impacts at each stage"

  - objective: "Calculate the CET1 impact of an IFRS 9 provision increase, including tax effects and IFRS 9 transitional arrangements"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student computes post-tax provision impact on CET1, applies transitional add-back, and determines whether the bank breaches its combined buffer requirement using Exercise 9 data"

  - objective: "Explain why pillar-isolated agents miss cross-pillar impacts and produce incomplete advice"
    proficiency_level: "B2"
    bloom_level: "Evaluate"
    assessment_method: "Student identifies at least three specific cross-pillar impacts that a siloed agent would miss and explains the regulatory and business consequences"

cognitive_load:
  new_concepts: 5
  concepts_list:
    - "Cross-pillar cascade: AML event triggers IFRS 9 stage migration triggers Basel capital impact"
    - "Provision increase reduces CET1 through retained earnings (post-tax: provision x (1 - tax rate) / RWA)"
...
- Router loads: `ifrs9-staging` + `ifrs9-ecl`
- No cross-pillar chaining needed

**Cross-pillar query**: "What is the capital impact of our ECL increase?"

- Router loads: `ifrs9-ecl` (get the ECL figure) -> `basel-capital` (compute CET1 impact) -> `basel-rwa` (check for RWA changes)
...
- Router loads: `aml-sar-drafting` (SAR workflow) -> `ifrs9-staging` (stage migration) -> `ifrs9-ecl` (ECL recalculation) -> `basel-capital` (CET1 impact) -> `bank-reconciliation` (ensure GL reflects changes)
- Five skills chained, each transforming the output of its predecessor
