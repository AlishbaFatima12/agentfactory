...
skills:
  - name: "Generate Context-Rich Meeting Prep Briefs Using Stakeholder Notes from work.local.md"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Communication"
    measurable_at_this_level: "Student can generate a meeting prep brief for a real upcoming meeting that includes per-agenda-item context, stakeholder communication notes drawn from work.local.md, decisions needed vs discussion items, and meeting rules from the organisational culture layer"

  - name: "Apply D/A/F/Q/R Note-Taking During Meetings to Produce Synthesis-Ready Output"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Communication"
    measurable_at_this_level: "Student can take meeting notes using D/A/F/Q/R codes during a real or simulated meeting, then generate a structured synthesis with numbered decisions (D-YYYY-NNN), owned actions with specific deadlines, and deferred items with triggers for revisit"

  - name: "Evaluate Meeting Synthesis Quality Against the Specificity Standard"
...
learning_objectives:
  - objective: "Describe the three-phase meeting model (before/during/after) and explain what each phase produces and why the pre-meeting preparation phase is the highest-leverage of the three"
    ...
    assessment_method: "Student can explain why stakeholder notes in the prep brief are more valuable than the agenda alone, and articulate what fails in meetings where people arrive without the prep brief (wrong discussions, uninformed decisions, actions that do not stick)"

  - objective: "Generate a meeting prep brief and a meeting synthesis for the same meeting, and trace how the D/A/F/Q/R notes become the structured output"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student produces a prep brief with per-agenda-item context and stakeholder notes, takes D/A/F/Q/R notes during a meeting (real or case study), and generates a synthesis where every decision is numbered and specific, every action has one named owner and a specific date, and every deferred item has a trigger"

  - objective: "Configure the decision numbering convention and explain why persistent numbered decisions create a searchable organisational history"
    proficiency_level: "B2"
    bloom_level: "Analyze"
    assessment_method: "Student can explain the D-YYYY-NNN / A-YYYY-NNN convention, demonstrate how decisions are stored in the work.local.md decision log, and describe a scenario where being able to search 'when did we decide X and why' would change a current decision"
...
teaching_guide:
  ...
  teaching_tips:
    - "The contrast between D-001/D-002/D-003 from the sample synthesis and the corresponding raw notes is the lesson's anchor. Show both side by side: 'Nighthawk: no update from liaison; decided to escalate to COO-level' becomes D-001 with a specific action, owner, and deadline. The transformation is obvious and immediately legible."
    - "Walk through the synthesis quality rubric with a deliberately bad example: 'The team discussed the budget and will look into it.' Have students identify each failure: vague decision, no owner, no deadline, no specificity. Then fix it using the quality standard."
---
...
Continue to [Lesson 10: The Executive Dashboard →](./10-the-executive-dashboard.md)
