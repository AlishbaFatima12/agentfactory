...
skills:
  - name: "Load Cross-Domain Context for Multi-Function Scenarios Using /agentic-office:context-loader"
  ...
  - name: "Run Cross-Context Searches Across All Four Memory Layers Using /agentic-office:workplace-search"
    ...
    measurable_at_this_level: "Student can run a cross-context search for a scenario that spans multiple memory layers, interpret results grouped by layer (project memory, meeting notes/decision log, people memory, terminology, task/delegation log), and use the recommended next step to take a specific action"

  - name: "Design Integration Protocols That Automate Cross-Domain Coordination"
    proficiency_level: "B2"
    category: "Conceptual"
    bloom_level: "Analyze"
    digcomp_area: "Problem-Solving"
...
learning_objectives:
  ...
  - objective: "Load cross-domain context for a multi-function scenario and identify what was surfaced that the student had not considered"
    proficiency_level: "B2"
    bloom_level: "Apply"
    assessment_method: "Student generates a cross-domain context brief for a real or case study scenario, then annotates it: which items were already known? Which were surfaced by the context load? Which gaps were identified that they had not noticed were gaps?"

  - objective: "Run the Dr. Sana Mirza onboarding scenario as a cross-domain integration exercise, identifying which domains are relevant and what the integrated action plan contains"
    proficiency_level: "B2"
    bloom_level: "Apply"
    assessment_method: "Student generates the cross-domain context brief for Dr. Sana Mirza's onboarding, correctly identifying Finance (headcount/budget), HR (onboarding protocol), and Operations (system access) as the relevant domains, and produces an integrated action plan that addresses all three"
...
| ----------------- | -------------------------------------------------------------------- | ---------------------------------------------------------- |
| **Single-domain** | Task primarily affects one domain, but you want all relevant context | Load finance context before reviewing the analytics budget |
| **Cross-domain**  | Task genuinely spans multiple domains simultaneously                 | Load HR + Finance + Ops for Dr. Sana Mirza's onboarding    |
| **Person**        | About to interact with someone; want their full context              | Load full context on Omar before the budget ROI review     |
| **Project**       | About to work on a project; want all cross-domain implications       | Load full context on Project Nighthawk                     |
| **Decision**      | About to make a decision; want all relevant context loaded           | Load context for the Islamabad expansion decision          |
...
| Action                                              | Domain            | Owner          | Due               |
| --------------------------------------------------- | ----------------- | -------------- | ----------------- |
| Submit system access provisioning request           | Operations        | Zia            | Thursday 20 March |
| Brief introduction to Omar (before Monday)          | People            | Zia            | Friday 21 March   |
| Prepare written welcome brief for Dr. Sana Mirza    | HR/People         | Zia            | Friday 21 March   |
| Coordinate 30-day check-in scheduling with Ayesha's | HR                | Zia            | Week 1            |
...
Continue to [Lesson 12: The Digital Chief of Staff →](./12-the-digital-chief-of-staff.md)
