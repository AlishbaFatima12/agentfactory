...
skills:
  - name: "Generate People Analytics Reports Using /people-report"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Information"
    measurable_at_this_level: "Student can invoke /people-report with workforce data to generate a headcount, attrition, or org health report, and interpret the key metrics to surface at least two data-driven recommendations"

  - name: "Analyse Recruiting Pipeline Velocity and Conversion Rates"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Analyze"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can use /recruiting-pipeline to analyse stage-by-stage conversion rates and time-in-stage for an active role, and identify the specific stage causing the most friction"

  - name: "Read Agent Reports as Operational Intelligence"
...
learning_objectives:
  - objective: "Use /people-report to generate a quarterly headcount and attrition report for a 200-person organisation"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student runs /people-report with scenario data, receives the structured output, and identifies the two most significant findings and the recommended action for each"

  - objective: "Use /recruiting-pipeline to diagnose a slow-moving hiring process by identifying the stage with the highest time-in-stage"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Student analyses pipeline data for an active role, identifies which stage is causing the most delay, and proposes one specific intervention to improve conversion at that stage"

  - objective: "Interpret a knowledge-base-agent weekly report to identify FAQ gaps, accuracy concerns, and escalation patterns"
    proficiency_level: "B2"
    bloom_level: "Evaluate"
    assessment_method: "Student reviews a sample KB agent report, identifies at least two gaps requiring documentation action, one escalation pattern requiring HR review, and rates the agent's current accuracy for the types of queries it is handling"

  - objective: "Design an HR intelligence dashboard mapping all four agent outputs to specific business questions and action thresholds"
    proficiency_level: "B2"
    bloom_level: "Create"
    assessment_method: "Student produces a one-page dashboard design showing which metrics from which agents they would monitor weekly vs monthly, what thresholds trigger action, and who is responsible for each signal"
...
differentiation:
  ...
  remedial_for_struggling: "Focus on one agent report at a time. Start with the knowledge-base-agent weekly report: read it as if you are the CHRO, identify the three most important signals, and decide what you would do about each one. Once you can read one agent's report as operational intelligence, the others follow the same pattern."
...
| Report Type    | Business Question                                           | Key Metrics                                                             |
| -------------- | ----------------------------------------------------------- | ----------------------------------------------------------------------- |
| **Headcount**  | Where are we? Who do we have, doing what, where?            | Total headcount by team, level, location; tenure distribution           |
| **Attrition**  | Are we keeping people? Who is leaving, from where, and why? | Voluntary attrition rate; regrettable attrition; average tenure at exit |
| **Diversity**  | Is our talent pipeline representative?                      | Representation by level; promotion rates by group; pipeline diversity   |
| **Org Health** | Is our structure working? Are there structural risks?       | Span of control; management layers; flight risk indicators; eNPS        |
...
| Stage         | Description                   | If Delayed, Signals...                                    |
| ------------- | ----------------------------- | --------------------------------------------------------- |
| **Sourced**   | Identified and reached out    | Weak sourcing or unappealing outreach                     |
| **Screen**    | Phone or video screen         | JD attracted wrong candidates; screening criteria unclear |
| **Interview** | Panel or structured interview | Scheduling friction; hiring manager availability          |
| **Debrief**   | Team calibration and decision | Calibration misalignment; decision-making confusion       |
| **Offer**     | Extending the offer           | Approval bottlenecks; compensation concerns               |
| **Accepted**  | Offer accepted                | Competitive counteroffer; candidate experience failure    |
...
| Signal                                | What It Means                                               | Action                                      |
| ------------------------------------- | ----------------------------------------------------------- | ------------------------------------------- |
| Statutory rate change detected        | A mandatory rate (minimum wage, sick pay, etc.) has changed | Review update before it goes live           |
| Cross-reference inconsistency flagged | Two policies contradict each other                          | HR review and resolution                    |
| Policy last-reviewed date >12 months  | Policy is overdue for review                                | Schedule a review with the relevant HR lead |
