...
skills:
  - name: "Interpret Failure Pattern Clusters"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Analyse"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can identify where failures cluster by scenario category, trace each cluster to the SKILL.md section that likely needs revision, and explain why different failure patterns indicate different types of SKILL.md problems"

  - name: "Perform Targeted SKILL.md Rewrites Without Regression"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Content Creation"
    measurable_at_this_level: "Student can rewrite the two weakest instructions in an affected SKILL.md section, re-run the scenario set against those scenarios, and check for regression in previously passing scenarios"

  - name: "Manage the Shadow-to-Autonomous Transition"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Analyse"
    digcomp_area: "Safety"
    measurable_at_this_level: "Student can describe the shadow mode protocol (30 days minimum, 95% accuracy), explain what shadow mode validates that scenario testing cannot, and describe the graduated autonomy model"

learning_objectives:
  - objective: "Interpret failure pattern clusters from scenario testing and trace each cluster to the specific SKILL.md section that needs revision"
    proficiency_level: "B1"
    bloom_level: "Analyse"
    assessment_method: "Given a set of scenario test results with failures, student can identify the failure pattern, trace it to a SKILL.md section (Persona, Questions, or Principles), and explain the connection"

  - objective: "Perform a targeted SKILL.md rewrite that resolves identified failures without introducing regressions in previously passing scenarios"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Given a failing scenario and the relevant SKILL.md section, student can identify the two weakest instructions, propose rewrites, and describe how to check for regression"

  - objective: "Describe the shadow mode protocol, explain what it validates beyond scenario testing, and describe the graduated autonomy model for transitioning to autonomous operation"
    proficiency_level: "B1"
    bloom_level: "Analyse"
    assessment_method: "Student can state the shadow mode criteria, explain why 30 days of production inputs are necessary beyond scenario testing, and describe the staged transition from partial to full autonomy"

cognitive_load:
  new_concepts: 4
  concepts_list:
    - "Failure pattern interpretation (clusters by category reveal SKILL.md section problems)"
...
differentiation:
  ...
  remedial_for_struggling: "Focus on the failure pattern interpretation. If most failures are in standard cases, the SKILL.md's core function is unclear. If most failures are in edge cases, the boundaries are unclear. If most failures are in adversarial cases, the Principles are too vague. If most failures are in high-stakes cases, the escalation logic is broken. One sentence per pattern is sufficient."
