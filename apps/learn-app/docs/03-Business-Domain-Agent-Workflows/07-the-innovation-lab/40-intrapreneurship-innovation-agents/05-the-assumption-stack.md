---
slug: /Business-Domain-Agent-Workflows/intrapreneurship-innovation-agents/the-assumption-stack
sidebar_position: 5
title: "The Assumption Stack"
description: "Map every assumption embedded in your selected idea using the three-tier risk framework, score evidence quality, and design the cheapest possible test for each assumption using the /hypothesis skill"
keywords:
  [
    "assumption map",
    "lean startup",
    "hypothesis testing",
    "assumption risk tiers",
    "minimum viable test",
    "evidence quality",
    "validated learning",
    "MVT hierarchy",
...
skills:
  - name: "Build a three-tier assumption map for a venture across five assumption categories"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Analyze"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can produce a structured assumption map with 20+ assumptions across Customer, Problem, Solution, Business Model, and Technical categories, each assigned to TIER 1/2/3 with a rationale for the tier assignment"

  - name: "Score assumption evidence quality and design minimum viable tests ordered by cost"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can assign ASSUMED/ANECDOTAL/VALIDATED status to each assumption and identify the cheapest test at the appropriate MVT level (Conversation through Functional MVP) that would move the assumption from ASSUMED to VALIDATED"

learning_objectives:
  - objective: "Classify venture assumptions into three risk tiers using the 'direction-change' heuristic"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Student assigns tier levels to 20+ assumptions and provides a rationale for each TIER 1 assignment that explicitly answers: 'If this is wrong, would we change direction?' The rationale must reference a specific failure mode (no revenue, cannot build, cannot acquire customers)"
  ...
  - objective: "Distinguish between ASSUMED, ANECDOTAL, and VALIDATED evidence quality for venture assumptions"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Student correctly categorises evidence for 10+ assumptions: VALIDATED requires specific behavioural evidence (N customers paid $X or used it N times); ANECDOTAL requires a named source with context; ASSUMED requires no external evidence"
