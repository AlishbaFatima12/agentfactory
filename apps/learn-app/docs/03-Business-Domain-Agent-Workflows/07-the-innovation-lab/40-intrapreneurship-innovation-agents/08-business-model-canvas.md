---
slug: /Business-Domain-Agent-Workflows/intrapreneurship-innovation-agents/business-model-canvas
sidebar_position: 8
title: "Business Model Canvas"
description: "Map the nine building blocks of your venture using Osterwalder's Business Model Canvas, assess evidence quality per block, stress-test with adversarial scenarios, and explore alternative business model configurations using the /canvas skill"
keywords:
  [
...
skills:
  - name: "Build a Business Model Canvas with Evidence Quality Assessment"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can complete all 9 canvas blocks with a hypothesis, evidence quality rating (ASSUMED/ANECDOTAL/VALIDATED), biggest risk, and open question , and produce a health summary identifying validated, partially validated, and hypothetical blocks"

  - name: "Stress-Test a Business Model Canvas with Adversarial Scenarios"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Analyze"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can design adversarial scenarios targeting the weakest canvas blocks, interpret impact assessments and mitigation strategies, and adjust canvas assumptions based on stress-test findings"

  - name: "Explore Alternative Business Model Configurations"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Evaluate"
    digcomp_area: "Content-Creation"
    measurable_at_this_level: "Student can generate three structurally distinct business model alternatives (e.g. subscription vs. usage-based vs. marketplace), articulate the biggest advantage and fatal flaw of each, and make a documented choice with rationale"

learning_objectives:
  - objective: "Complete all 9 canvas blocks with evidence quality ratings and produce a canvas health summary identifying validated, partially validated, and hypothetical blocks"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student produces a canvas where every block includes evidence quality (ASSUMED/ANECDOTAL/VALIDATED), biggest risk, and one open question , and the health summary correctly categorises each block"

  - objective: "Stress-test the canvas with three adversarial scenarios targeting the most fragile blocks"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Student produces impact assessments and mitigation strategies for three scenarios, and identifies which scenario poses the greatest existential risk with a rationale grounded in canvas evidence quality"

  - objective: "Generate three alternative business model configurations and select the primary model with documented reasoning"
    proficiency_level: "B1"
    bloom_level: "Evaluate"
    assessment_method: "Student produces three distinct alternatives (not just pricing variations) with advantage/fatal flaw analysis and a clear selection rationale referencing canvas evidence and constraints"

cognitive_load:
  new_concepts: 4
  concepts_list:
    - "9-block Business Model Canvas: the integrated map of how a business creates and captures value"
    - "Evidence quality rating, ASSUMED / ANECDOTAL / VALIDATED and what each means for risk"
    - "Canvas health summary, which blocks are proven vs. hypothetical and where to focus next"
    - "Adversarial stress-testing, designing scenarios that attack the weakest canvas assumptions"
  assessment: "4 concepts at B1 level is within the cognitive limit. The canvas builds naturally on prior lessons: Customer Segments come from L03, Value Propositions from L04-L05, Revenue Streams from L07. Most blocks will feel familiar; the new skill is structuring and rating evidence quality per block."

differentiation:
  extension_for_advanced: "After completing the AP canvas, run a canvas for a second business model configuration entirely, choose a structurally different model (e.g. marketplace instead of SaaS) and complete all 9 blocks. Compare: which blocks become stronger? Which become weaker? What does the comparison reveal about the assumptions your primary model depends on most?"
  remedial_for_struggling: "Focus on three blocks first: Customer Segments (who are you building for?), Value Propositions (what specific outcome do you deliver?), and Revenue Streams (how does money change hands?). If you can complete these three with honest evidence quality ratings, you understand the canvas framework. The remaining six blocks elaborate on these foundations."

teaching_guide:
  key_points:
    - "The canvas is a hypothesis map, not a business plan: every block is an assumption until customers validate it"
    - "Evidence quality (ASSUMED/ANECDOTAL/VALIDATED) is as important as the block content: a canvas without evidence ratings is a wishlist"
    - "The health summary reveals where execution risk is concentrated, validated blocks are assets; hypothetical blocks are liabilities"
    - "L08 uses post-pilot data from L07: the canvas should reflect validated learning, not initial assumptions"
    - "Stress-testing is how you find your dependencies before a competitor or market does"
  misconceptions:
    - "The canvas is a one-time exercise. Correction: the canvas should be versioned and updated after every significant learning event (pilot results, customer conversation, market change). A canvas from Month 1 that has not been updated in Month 6 is a liability, not an asset."
    - "All 9 blocks are equally important. Correction: the health summary reveals which blocks carry the most unvalidated risk. A hypothetical Key Partnerships block in a platform business is existential; a hypothetical Key Partnerships block in a direct SaaS business is a lower priority."
    - "A strong value proposition means the canvas is solid. Correction: the value proposition is only one of nine blocks. Many ventures with strong value propositions fail because their channel assumptions or cost structure assumptions were never tested."
  discussion_prompts:
    - "Look at the Key Partnerships block for AP automation; it is entirely hypothetical. If ERP partnerships are critical to enterprise deals, what is the cost of that hypothesis being wrong? What would you do in Month 1 to test it?"
    - "The canvas has three blocks with evidence marked ANECDOTAL (Channels, Cost Structure, Revenue Streams). Which of these carries the most existential risk if the assumption is wrong , and why?"
  teaching_tips:
    - "Show the canvas version number (Version 1.2, post-MVP) prominently ; it signals that this is a living document, not a final answer. Ask students: what event would trigger Version 1.3?"
    - "The stress-test scenarios should attack the blocks students rated HYPOTHETICAL or ANECDOTAL, not the VALIDATED ones. If a student stress-tests a validated block, redirect them: 'Your most dangerous scenario is the one you haven't planned for.'"
---
...
:::info Reading the Health Summary
...
:::note Your stress-test output will vary
...
**Time:** 90 minutes
**Goal:** Build a complete, validated Business Model Canvas for your venture from Exercise 3 (Lean Startup), stress-test the most fragile blocks, and explore alternative configurations
...
Continue to [Lesson 9: Unit Economics and Financial Modelling →](./09-unit-economics-financial-modelling.md)
