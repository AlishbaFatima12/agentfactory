...
Your FastAPI agent from Chapter 79 is containerized and pushed to a registry. Now deploy it to a production-like environment using Kubernetes.
...
**Container image**: This is the image you pushed in Chapter 79. Remember the format:
```
...
**Port**: Your FastAPI agent listens on port 8000 by default (unless you configured differently in Chapter 79).

**Environment variables**: Your agent needs:
- `OPENAI_API_KEY`: Your OpenAI API key (should be in a Secret for security)
- Any other config from Chapter 79? (Database URLs, model names, API endpoints?)
