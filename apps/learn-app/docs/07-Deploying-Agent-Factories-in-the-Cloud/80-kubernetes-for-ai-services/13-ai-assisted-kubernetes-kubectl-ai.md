...
To show how kubectl-ai works in practice, let's walk through a realistic scenario: deploying your FastAPI agent from Chapter 79 to Kubernetes.
...
**Setup**: You have a containerized FastAPI agent from Chapter 79 (image: `my-agent:1.0` on Docker Hub). You need to deploy it to Kubernetes with the following requirements:
