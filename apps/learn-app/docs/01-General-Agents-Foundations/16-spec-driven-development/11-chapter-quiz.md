...
{
question: "A specification is followed perfectly, but the resulting code has 80-line functions, no type hints, and inconsistent error messages. What is missing?",
options: [
"A more detailed specification with implementation examples",
"A project constitution defining coding standards",
"A code review step after implementation",
...
{
question: "When should the project constitution be written relative to the four SDD phases?",
options: [
"After Phase 1 Research, informed by findings",
"During Phase 2 Specification, alongside the first spec",
"Before Phase 1 Research begins",
...
{
question: "Two subagents implement tasks in parallel. Without a constitution, Subagent 1 uses async/await while Subagent 2 uses callbacks. Both follow the spec correctly. What problem does this illustrate?",
options: [
"The specification was underspecified for implementation details",
"Cross-agent inconsistency from missing shared governance",
"The subagents should have communicated during implementation",
...
{
question: "Claude Code reads CLAUDE.md files at which locations?",
options: [
"Only the project root CLAUDE.md file",
"Global (~/.claude/CLAUDE.md), project root, and relevant subdirectories",
"Only the global ~/.claude/CLAUDE.md file",
