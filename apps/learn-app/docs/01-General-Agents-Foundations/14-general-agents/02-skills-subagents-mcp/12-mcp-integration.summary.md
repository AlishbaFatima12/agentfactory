...
- Install MCP servers with `claude mcp add <name> --transport stdio -- npx -y <package>` (note the `--` separator and `-y` flag)
- Verify registered servers with `claude mcp list`
- For team projects, use `.mcp.json` at the project root (committable to version control) instead of per-machine CLI setup
...
- MCP Tool Search is included by default (no version requirement); activates when tool definitions exceed 10% of context
