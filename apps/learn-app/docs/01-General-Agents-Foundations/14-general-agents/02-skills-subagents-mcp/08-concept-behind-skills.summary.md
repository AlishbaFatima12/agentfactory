...
1. **Bundled**: Ship with Claude Code (`/batch`, `/debug`, `/simplify`, `/loop`, `/claude-api`)
2. **Foundational**: Basic capabilities (document creation, PDF handling)
3. **Partner/Third-Party**: Software-specific expertise (browser automation, Notion)
4. **Enterprise/Custom**: Organizational knowledge (coding standards, internal workflows)

### Portability

- Skills follow the Agent Skills open standard (agentskills.io)
- Portable across Claude Code, OpenAI Codex, Goose, and other compatible tools
- Skills are not locked to a single vendor
