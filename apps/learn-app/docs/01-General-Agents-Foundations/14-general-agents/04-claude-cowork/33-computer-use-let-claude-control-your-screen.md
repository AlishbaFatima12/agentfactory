---
slug: /General-Agents-Foundations/general-agents/computer-use-let-claude-control-your-screen
title: "Computer Use: Let Claude Control Your Screen"
sidebar_position: 33
chapter: 14
lesson: 33
duration_minutes: 30
chapter_type: Practical
running_example_id: cowork-computer-use

# PEDAGOGICAL LAYER METADATA
primary_layer: "Layer 2"
layer_progression: "L2 (Collaboration)"
layer_1_foundation: "N/A"
layer_2_collaboration: "Students enable Computer Use, grant permissions, observe Claude controlling screen elements, and evaluate safety boundaries for autonomous screen interaction"
layer_3_intelligence: "N/A"
layer_4_capstone: "N/A"

# HIDDEN SKILLS METADATA (Institutional Integration Layer)
skills:
  - name: "Computer Use Configuration"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Digital Content Creation"
    measurable_at_this_level: "Student can enable Computer Use, grant macOS accessibility and screen recording permissions, and configure per-app access tiers"

  - name: "Autonomous Screen Interaction Safety"
    proficiency_level: "B1"
    category: "Conceptual"
    bloom_level: "Evaluate"
    digcomp_area: "Safety"
    measurable_at_this_level: "Student can evaluate which applications should receive full control, click-only, or view-only access and articulate the risks of each tier"

  - name: "Digital FTE Screen Delegation"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Problem Solving"
    measurable_at_this_level: "Student can delegate a multi-step screen interaction task to Claude and verify the result matches the intended outcome"

learning_objectives:
  - objective: "Enable Computer Use and grant the required macOS permissions (Accessibility, Screen Recording)"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Successful toggle of Computer Use with both permissions granted"
  - objective: "Execute a screen interaction task by instructing Claude to open an application, perform actions, and return a result"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Claude completes a multi-step screen task with observable results"
  - objective: "Evaluate per-app permission tiers and decide which applications should receive which level of access"
    proficiency_level: "B1"
    bloom_level: "Evaluate"
    assessment_method: "Student explains the rationale for allowing or denying specific applications"

cognitive_load:
  new_concepts: 5
  assessment: "5 concepts (computer use toggle, accessibility permissions, screen recording permissions, per-app permission tiers, tool priority hierarchy) within B1 limit of 7"

differentiation:
  extension_for_advanced: "Chain computer use with a connector: have Claude research something in Safari, then use the Gmail connector to email the summary. Compare what happened via screen control vs what happened via connector."
  remedial_for_struggling: "Start with a single-app task (open Calculator, perform one calculation). Focus on the permission flow before attempting multi-app workflows."
...
teaching_guide:
  ...
  session_group: 9
  session_title: "Computer Use and Screen Control"
  key_points:
    - "Computer Use is the broadest but slowest tool in Claude's hierarchy: connectors beat browser, browser beats screen control. Students must understand when screen control is the right choice."
    - "The three permission tiers (view only, click only, full control) are fixed by app category, not user choice. Browsers are always view-only. Terminals are always click-only. Everything else gets full control."
    - "Safety is not optional: Computer Use runs on your real desktop, not in a sandbox. Anything visible on screen is accessible to Claude through screenshots."
    - "The 'wow factor' of watching Claude click through your apps is the most visceral Digital FTE moment in the course. Let students sit with that experience before rushing to the next topic."
  misconceptions:
    - "Students think Computer Use is the best way to do everything. In reality, connectors and browser tools are faster and more reliable. Computer Use is the fallback for apps that have no API or CLI."
    - "Students assume they can change an app's permission tier (e.g., give Terminal full control). The tiers are fixed by app category and cannot be overridden."
    - "Students believe Computer Use works on Windows or Linux. It is macOS-only (Apple Silicon) as of this writing."
  discussion_prompts:
    - "You have three tools: a Gmail connector, Chrome browser control, and Computer Use. Your task is to find a restaurant, check reviews, and email the link to a friend. Which tool handles which step, and why?"
    - "What would you add to your denied-apps list, and what is the worst case if you forgot to deny an app that should be restricted?"
  teaching_tips:
    - "Run a live demo of Computer Use on a simple task (open Notes, type a sentence, take a screenshot). The visual impact of watching Claude move the cursor is more persuasive than any explanation."
    - "Have students write their denied-apps list BEFORE enabling Computer Use. This forces them to think about safety proactively rather than reactively."
    - "If students do not have Apple Silicon Macs, pair them with someone who does, or show a screen recording. The concepts (permission tiers, tool hierarchy, safety boundaries) transfer even without hands-on access."
  assessment_quick_check:
    - "Name the three per-app permission tiers and give one example app for each."
    - "Why does Claude try connectors and browser tools before falling back to Computer Use?"
    - "What two macOS permissions must be granted before Computer Use works?"
---
...
| **App**      | Claude Desktop, latest version from [claude.com/download](https://claude.com/download) |

:::caution Windows and Linux
...
1. When prompted, click the badge in Claude Settings to open **System Settings > Privacy & Security > Accessibility**
2. Find **Claude** in the list
3. Toggle it on
4. You may need to enter your Mac password or use Touch ID to confirm

### Screen Recording (see your screen)

This permission lets Claude take screenshots to understand what is on screen.

1. Click the badge in Claude Settings to open **System Settings > Privacy & Security > Screen Recording**
2. Find **Claude** in the list
3. Toggle it on
...
1. **Take a screenshot** of your current desktop to understand the layout
2. **Open Calculator** (using Spotlight or the Applications folder)
3. **Click the number buttons** to enter 847 x 23
4. **Read the result** from another screenshot
5. **Report the answer** back in the chat
...
Claude may still _indirectly_ affect a denied app (for example, clicking a link that opens in a blocked browser), but it cannot interact with the denied app directly.
...
1. **Close sensitive apps.** If your banking app is open, Claude can see it in screenshots even if you deny direct access.
2. **Build your denied-apps list first.** Decide what is off-limits before you start, not after Claude asks for access.
3. **Use session-scoped approvals.** Permissions last for the current session only (or 30 minutes in Dispatch sessions). You approve fresh each time.
4. **Stay nearby for high-stakes tasks.** You can stop Claude at any point by clicking the stop button or typing a correction.

:::caution Research Preview
...
2. **Analyze**: identify UI elements, text, and layout
3. **Decide**: determine the next action (click, type, scroll, open app)
4. **Act**: execute the action
5. **Screenshot again**: verify the action produced the expected result
6. **Repeat** until the task is complete or Claude needs your input
