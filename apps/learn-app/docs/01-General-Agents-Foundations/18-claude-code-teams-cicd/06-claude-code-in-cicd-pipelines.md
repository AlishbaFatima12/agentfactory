---
slug: /General-Agents-Foundations/claude-code-teams-cicd/claude-code-in-cicd-pipelines
sidebar_position: 6
title: "Claude Code in CI/CD Pipelines"
description: "Run Claude Code non-interactively in CI with the -p flag, produce structured JSON output with --json-schema, and build a working GitHub Actions workflow for automated code review and test generation"
keywords:
  [
    claude code,
    CI/CD,
...
layer_2_collaboration: "Students configure Claude Code to run autonomously in CI pipelines, produce machine-parseable structured output, and handle practical CI concerns like duplicate comments and session isolation"
layer_3_intelligence: "N/A"
layer_4_capstone: "N/A"

# HIDDEN SKILLS METADATA
skills:
  - name: "Non-Interactive Claude Code Execution"
    proficiency_level: "B1"
    category: "Technical"
    bloom_level: "Apply"
    digcomp_area: "Digital-Content-Creation"
    measurable_at_this_level: "Student can run Claude Code with the -p flag in a script, pass a prompt, and capture the output for downstream processing"

  - name: "Structured CI Output with JSON Schema"
    proficiency_level: "B1"
    category: "Technical"
    bloom_level: "Apply"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can use --output-format json with --json-schema to produce machine-parseable structured output conforming to a specific schema"

  - name: "CLAUDE.md as CI Context"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Digital-Content-Creation"
    measurable_at_this_level: "Student can write a CLAUDE.md file that provides testing standards, review criteria, and project conventions to CI-invoked Claude Code"

  - name: "CI Workflow Design for Claude Code"
    proficiency_level: "B2"
    category: "Applied"
    bloom_level: "Create"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can build a complete GitHub Actions workflow that runs Claude Code on pull requests, produces structured review findings, and avoids duplicate comments on re-runs"

learning_objectives:
  - objective: "Run Claude Code in CI with the -p flag to process a prompt non-interactively and print the result to stdout"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student runs claude -p with a prompt in a terminal script, captures the output, and verifies it completed without waiting for interactive input"
  - objective: "Use --output-format json with --json-schema to produce structured CI output that conforms to a defined schema"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student writes a JSON schema for review findings, runs Claude with that schema, and parses the structured_output field from the response"
  - objective: "Write a CLAUDE.md with testing standards and review criteria that guides CI-invoked Claude Code"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student creates a CLAUDE.md with specific review criteria and observes that Claude's CI output follows those criteria"
  - objective: "Include prior review findings in context to avoid duplicate PR comments when re-running reviews"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student demonstrates a workflow that feeds previous review comments back into the next run and produces only new or unresolved findings"
  - objective: "Build a working GitHub Actions workflow that runs Claude Code on pull requests"
    proficiency_level: "B2"
    bloom_level: "Create"
    assessment_method: "Student creates a .github/workflows YAML file that triggers on PR events, runs Claude Code with -p, and posts findings as PR comments"

cognitive_load:
  new_concepts: 5
  assessment: "5 concepts (-p flag for non-interactive mode, structured output with JSON schema, CLAUDE.md as CI context, duplicate comment avoidance, GitHub Actions workflow); within B2 limit of 10"

differentiation:
  extension_for_advanced: "Add a second CI job that runs Claude Code for test generation: provide existing test files in context so Claude avoids suggesting duplicate test scenarios. Compare the generated tests to what a human reviewer would suggest."
  remedial_for_struggling: "Start with claude -p locally in the terminal before attempting GitHub Actions. Confirm structured output works on your machine first, then move to the workflow YAML."
...
teaching_guide:
  ...
  session_group: 3
  session_title: "Claude Code in CI/CD Pipelines"
  key_points:
    - "The -p (or --print) flag is the ONLY correct way to run Claude Code non-interactively; other approaches (environment variables, stdin redirect) are not supported"
    - "--output-format json returns metadata (session_id, usage) with the result; --json-schema puts structured output in the structured_output field"
    - "CLAUDE.md loaded by CI-invoked Claude Code provides the same project context as an interactive session: testing standards, review criteria, code conventions"
    - "Session isolation matters: the same session that generated code is biased toward its own decisions and less likely to catch its own bugs during review"
    - "Duplicate comment avoidance requires feeding prior review findings back as context with instructions to report only new or still-unresolved issues"
  misconceptions:
    - "Students think there is a CLAUDE_HEADLESS environment variable or --batch flag; the -p flag is the only documented non-interactive mode"
    - "Students assume --output-format json returns ONLY the structured output; it returns a wrapper with session_id, usage stats, and the result or structured_output field"
    - "Students expect CI-invoked Claude Code to automatically know project conventions; without CLAUDE.md or --append-system-prompt, it has no project-specific context beyond the code it reads"
  discussion_prompts:
    - "Why is it important that the reviewer session is separate from the generator session? What biases does a generator session carry?"
    - "What testing standards would you put in CLAUDE.md for your own project's CI pipeline?"
  teaching_tips:
    - "Run the -p flag locally first so students see the output format before tackling GitHub Actions YAML"
    - "The exam Q10 scenario (pipeline hanging) is a great hook: most students have experienced a CI job that hangs waiting for input"
    - "Show the JSON output with and without --json-schema so students see the difference between the result field (free text) and structured_output field (schema-conforming)"
  assessment_quick_check:
    - "Your CI pipeline runs 'claude Analyze this PR' and hangs. What flag is missing?"
    - "What is the difference between the result field and the structured_output field in --output-format json?"
    - "Your overnight technical debt report does not need to finish in 30 seconds. Should you use real-time Claude API calls or the Message Batches API?"
---
