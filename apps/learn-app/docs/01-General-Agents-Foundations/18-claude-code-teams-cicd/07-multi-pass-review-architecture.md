---
slug: /General-Agents-Foundations/claude-code-teams-cicd/multi-pass-review-architecture
sidebar_position: 7
title: "Multi-Pass Review Architecture"
description: "Build a multi-pass code review workflow using independent Claude instances, per-file analysis passes, cross-file integration passes, and confidence self-reporting"
keywords:
  [
    multi-pass review,
    code review,
...
layer_2_collaboration: "Building multi-pass review workflows with independent Claude instances, writing specific review criteria, interpreting confidence scores"
layer_3_intelligence: "N/A"
layer_4_capstone: "N/A"

# HIDDEN SKILLS METADATA
skills:
  - name: "Self-Review Limitation Analysis"
    proficiency_level: "B1"
    category: "Conceptual"
    bloom_level: "Analyze"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can explain why a generator session reviewing its own output produces lower-quality reviews than an independent instance"

  - name: "Multi-Pass Review Design"
    proficiency_level: "B2"
    category: "Applied"
    bloom_level: "Create"
    digcomp_area: "Digital-Content-Creation"
    measurable_at_this_level: "Student can design and implement a review workflow that splits a large PR into per-file passes plus a cross-file integration pass"

  - name: "Review Criteria Specification"
    proficiency_level: "B1"
    category: "Technical"
    bloom_level: "Apply"
    digcomp_area: "Information-Data-Literacy"
    measurable_at_this_level: "Student can write specific, categorical review criteria that define what to report and what to skip, replacing vague instructions like 'be conservative'"

  - name: "Confidence-Calibrated Review Routing"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Evaluate"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can interpret confidence self-reports alongside review findings and use them to prioritize human attention"

learning_objectives:
  - objective: "Explain why self-review fails and why an independent Claude instance catches issues the generator misses"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Student articulates the reasoning-context retention problem and demonstrates using a separate session for review"
  - objective: "Split a multi-file PR review into per-file local analysis passes plus a cross-file integration pass"
    proficiency_level: "B2"
    bloom_level: "Create"
    assessment_method: "Student writes a shell script that runs per-file reviews in parallel and a final cross-file pass, producing structured output"
  - objective: "Write specific review criteria that define which issues to report and which to skip, replacing vague instructions"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student converts a vague review prompt into categorical criteria with concrete code examples for each severity level"
  - objective: "Implement confidence self-reporting alongside review findings to enable calibrated routing of human attention"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student adds confidence fields to structured review output and explains how to use them for triage"

cognitive_load:
  new_concepts: 5
  assessment: "5 concepts (self-review limitation, session isolation for review, per-file vs cross-file passes, specific review criteria, confidence self-reporting); within B1-B2 limit"

differentiation:
  extension_for_advanced: "Add a third pass that checks review findings against the project's test suite to verify whether flagged issues are already covered by tests"
  remedial_for_struggling: "Start with the Writer/Reviewer two-session pattern before attempting the full multi-pass script; focus on understanding why separate sessions matter"
...
teaching_guide:
  ...
  session_group: 4
  session_title: "Multi-Pass Review Architecture"
  key_points:
    - "Self-review fails because the generator session retains its reasoning context and is less likely to question its own decisions"
    - "An independent Claude instance with no prior context catches issues the generator misses, the same way a fresh pair of human eyes does"
    - "Per-file passes prevent attention dilution; a cross-file integration pass catches issues that only appear when files interact"
    - "Specific review criteria ('flag null pointer dereferences, SQL injection, unvalidated user input') outperform vague instructions ('be conservative') by giving Claude categorical targets"
    - "Confidence self-reporting lets you route human attention to low-confidence findings rather than reviewing everything"
  misconceptions:
    - "Students think self-review is fine because 'Claude is objective': Claude retains the reasoning that led to the original code and is biased toward confirming it"
    - "Students assume more context is always better: feeding an entire 14-file diff into one prompt causes attention dilution, missing issues in files reviewed later"
    - "Students think 'high confidence' means 'correct': confidence reflects how certain Claude is about the finding, not whether it is actually a real bug"
  discussion_prompts:
    - "Why does a human code reviewer catch bugs the author missed, even when the author re-reads their own code?"
    - "If you had to review a 50-file PR, would you read all 50 files in one sitting or break them into groups? Why?"
  teaching_tips:
    - "The Writer/Reviewer two-terminal demo is the best opening: have students write code in one session and review it in another, then compare to self-review"
    - "Show the attention dilution problem concretely: paste a 14-file diff into one prompt and observe that Claude misses an obvious bug in file 12"
    - "The confidence calibration exercise is eye-opening: students discover that high-confidence findings are correct more often, making triage possible"
  assessment_quick_check:
    - "Why should the reviewer be a different Claude session than the code generator?"
    - "What are the two passes in multi-pass review, and what does each one catch?"
    - "Name two problems with the review instruction 'be conservative and only report high-confidence findings'"
---

# Multi-Pass Review Architecture

You finish a 14-file pull request at 11 PM. You are tired, but the deadline is tomorrow. You re-read your own code, see nothing wrong, and merge. The next morning, a teammate spots a null pointer dereference in file 12 that you looked straight at and missed. Why? Because when you re-read your own code, your brain fills in what you _intended_ to write rather than what you actually wrote. A fresh reviewer, with no memory of your reasoning, reads what is actually on the screen.
...
When Claude generates code in a session, the conversation history contains every reasoning step: why it chose this data structure, why it handled the edge case this way, why it skipped input validation here. When you then ask "review this code for bugs" in the same session, Claude reads the code _through the lens of its own reasoning_. It already "knows" why each decision was made, so it is less likely to flag something as suspicious.
...
| :----------------------- | :----------------- | :-------------------------------------------------------- |
| **Session A (Writer)**   | Generates the code | Full reasoning history, design decisions, failed attempts |
| **Session B (Reviewer)** | Reviews the code   | Only the code itself, no access to the writer's reasoning |
...
| :------------------------------ | :------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------- |
| **Per-file passes**             | One file at a time, full attention                             | Local bugs: null dereferences, SQL injection, missing error handling, logic errors within a single file                            |
| **Cross-file integration pass** | All files together, but only reviewing inter-file interactions | Integration issues: inconsistent interfaces, broken contracts between modules, missing imports, data flow errors across boundaries |
...
A larger context window does not solve attention dilution. The problem is not that Claude cannot _fit_ 14 files; it is that reviewing 14 files in sequence causes later files to receive less careful analysis. Research on LLM attention patterns shows that relevance scoring degrades as context length increases, even when all content fits within the window. Breaking the review into passes is the structural fix.
...
This instruction fails in two ways. First, "be conservative" does not tell Claude _what_ to look for, so it defaults to a generic scan that catches obvious issues and misses subtle ones. Second, "only report high-confidence findings" tells Claude to self-filter based on confidence, which suppresses real findings that happen to be subtle. A SQL injection that requires tracing data flow through three functions might be "low confidence" but critical if real.
...
| Confidence | Action                                                                                                          |
| :--------- | :-------------------------------------------------------------------------------------------------------------- |
| **High**   | Likely real. Auto-comment on the PR or fix automatically.                                                       |
| **Medium** | Probably real but needs human judgment. Flag for reviewer.                                                      |
| **Low**    | Might be real or might be a false positive. Batch these for periodic review rather than commenting on every PR. |

Confidence is not accuracy. A "high confidence" finding means Claude is fairly certain the issue exists, but it could still be wrong. A "low confidence" finding means Claude is uncertain, but the issue could still be critical if it turns out to be real. Use confidence for _prioritization_, not for deciding whether to look at all.
...
**What you're learning:** The concrete difference between self-review and independent review. The generator session has context about _why_ it wrote the code the way it did and is less likely to question those decisions. The fresh session reads only the code and evaluates it on its merits.
