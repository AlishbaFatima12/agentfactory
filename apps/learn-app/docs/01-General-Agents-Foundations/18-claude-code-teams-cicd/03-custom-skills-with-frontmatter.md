---
slug: /General-Agents-Foundations/claude-code-teams-cicd/custom-skills-with-frontmatter
sidebar_position: 3
title: "Custom Skills with Frontmatter"
description: "Create custom slash commands and skills with YAML frontmatter: context:fork for isolated execution, allowed-tools for read-only restrictions, and argument-hint for parameter prompting"
keywords:
  [
    claude code,
    skills,
...
layer_2_collaboration: "Creating skills and commands, configuring frontmatter fields, choosing between skills and CLAUDE.md"
...
skills:
  - name: "Custom Command Creation"
    ...
    measurable_at_this_level: "Student can create a project-scoped command in .claude/commands/ that the entire team can invoke via /command-name"

  - name: "Skill Frontmatter Configuration"
    proficiency_level: "B1"
    category: "Technical"
    bloom_level: "Apply"
    digcomp_area: "Digital-Content-Creation"
    measurable_at_this_level: "Student can configure context:fork, allowed-tools, and argument-hint in SKILL.md frontmatter and explain when each is appropriate"

  - name: "Skills vs CLAUDE.md Decision Making"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Analyze"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can determine whether a piece of configuration belongs in a skill (on-demand, task-specific) or CLAUDE.md (always-loaded, universal standards)"

learning_objectives:
  - objective: "Create a project-scoped command in .claude/commands/ that teammates can invoke for code review"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student creates a /review command file and verifies it appears in the / menu"
  - objective: "Build three custom skills using context:fork, allowed-tools, and argument-hint frontmatter fields"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student creates three SKILL.md files, each demonstrating a different frontmatter capability"
  - objective: "Create a personal skill variant in ~/.claude/skills/ that does not affect team members"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student creates a personal skill and confirms it is not visible in the project's .claude/skills/ directory"
  - objective: "Apply the skills vs CLAUDE.md decision framework to classify configuration scenarios"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Student correctly categorizes 5 configuration scenarios as skill or CLAUDE.md and explains the reasoning"

cognitive_load:
  new_concepts: 6
  assessment: "6 concepts (project-scoped commands, SKILL.md format, context:fork, allowed-tools, argument-hint, skills vs CLAUDE.md decision); within B1 limit of 10"

differentiation:
  extension_for_advanced: "Build a skill with supporting files (templates, scripts) and dynamic context injection using !`command` syntax"
  remedial_for_struggling: "Start with a single command file in .claude/commands/ before progressing to skills with frontmatter"
...
prerequisites:
  ...
  - "Lesson 2: Path-Specific Rules with Glob Patterns"

teaching_guide:
  lesson_type: "hands-on"
  session_group: 2
  session_title: "Custom Skills and Commands"
  key_points:
    - "Commands (.claude/commands/) and skills (.claude/skills/) both create /slash-commands; skills add optional frontmatter and supporting files"
    - "context:fork runs a skill in an isolated subagent so verbose output does not pollute the main conversation"
    - "allowed-tools restricts what Claude can do during skill execution, useful for creating read-only analysis skills"
    - "argument-hint provides autocomplete hints so developers know what parameters a skill expects"
    - "Project-scoped skills commit to version control; personal skills in ~/.claude/skills/ stay private"
  misconceptions:
    - "Students think commands and skills are completely different systems; they are merged, with skills adding optional features on top of the same mechanism"
    - "Students assume context:fork means the skill runs on a remote server; it runs locally in an isolated conversation context"
    - "Students think allowed-tools grants new permissions; it restricts the default set, creating a narrower tool palette"
  discussion_prompts:
    - "When would you want a skill that only Claude can invoke (user-invocable: false) versus one only you can invoke (disable-model-invocation: true)?"
    - "What team conventions would you put in CLAUDE.md versus packaging as an on-demand skill?"
  teaching_tips:
    - "Start with the simplest possible command (a one-line .claude/commands/ file) before introducing SKILL.md frontmatter"
    - "Have students test context:fork by running the same skill with and without it, then comparing what happens to their conversation context"
    - "The skills vs CLAUDE.md decision framework is the conceptual anchor; keep returning to it when students ask where something should live"
  assessment_quick_check:
    - "Where do you put a command file so every team member who clones the repo gets it?"
    - "What frontmatter field prevents a skill from polluting your main conversation with verbose output?"
    - "Name one scenario where CLAUDE.md is better than a skill, and one where a skill is better."
---

# Custom Skills with Frontmatter

Your team has a code review checklist. It lives in a wiki page that nobody reads. Every pull request, someone asks "did you check for SQL injection?" and someone else says "I forgot." The checklist exists, but it is not _where the work happens_.
...
| :------------------------- | :---------------------------- | :-------------------------------------- |
| Creates a `/slash-command` | Yes                           | Yes                                     |
| Shared via version control | Yes (project-scoped)          | Yes (project-scoped)                    |
| YAML frontmatter           | Supported                     | Supported                               |
| Supporting files           | No (single file only)         | Yes (directory with templates, scripts) |
| Auto-invocation by Claude  | Yes (by default)              | Yes (by default)                        |
...
| :--------- | :---------------------------------- | :---------------------------- |
| Enterprise | Managed settings (admin-configured) | All users in the organization |
...
| Project    | `.claude/commands/<name>.md`        | Everyone who clones this repo |
...
| Configuration Type       | Where It Belongs | Why                                               |
| :----------------------- | :--------------- | :------------------------------------------------ |
| Code style conventions   | CLAUDE.md        | Applies to every task, every time                 |
| Commit message format    | CLAUDE.md        | Should be consistent across all commits           |
| Code review checklist    | Skill            | Only needed during reviews, not every interaction |
| Test generation workflow | Skill            | On-demand action with specific parameters         |
| API naming conventions   | CLAUDE.md        | Universal standard that always applies            |
| Dependency audit         | Skill            | Periodic task, not needed during normal coding    |
| Error handling patterns  | CLAUDE.md        | Should be applied in every file Claude writes     |
| Architecture exploration | Skill            | Exploratory task that produces verbose output     |
...
| `disable-model-invocation: true` | Yes            | No                | Side-effect skills (deploy, commit, delete) |
