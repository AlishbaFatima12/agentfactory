---
slug: /General-Agents-Foundations/claude-code-teams-cicd/iterative-refinement-techniques
sidebar_position: 5
title: "Iterative Refinement Techniques"
description: "Use concrete I/O examples, test-driven iteration, and the interview pattern to progressively improve Claude Code output instead of hoping the first attempt is correct"
keywords:
  [
...
layer_2_collaboration: "Students apply three refinement techniques to real transformation tasks, experience the feedback loop between tests and implementation, and choose between single-message and sequential iteration strategies"
layer_3_intelligence: "N/A"
layer_4_capstone: "N/A"

# HIDDEN SKILLS METADATA
skills:
  - name: "Concrete I/O Example Specification"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Digital-Content-Creation"
    measurable_at_this_level: "Student can write 2-3 concrete input/output examples that disambiguate a transformation requirement Claude previously interpreted incorrectly"

  - name: "Test-Driven Iteration with Claude Code"
    proficiency_level: "B1"
    category: "Technical"
    bloom_level: "Apply"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can write a test suite first, share test failures with Claude, and iterate until all tests pass without manually writing the implementation"

  - name: "Interview Pattern for Unfamiliar Domains"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Analyze"
    digcomp_area: "Problem-Solving"
    measurable_at_this_level: "Student can prompt Claude to ask clarifying questions about a domain before implementing, and recognize when the interview surfaced considerations they had not anticipated"

  - name: "Single-Message vs Sequential Iteration Strategy"
    proficiency_level: "A2"
    category: "Conceptual"
    bloom_level: "Analyze"
    digcomp_area: "Information-Data-Literacy"
    measurable_at_this_level: "Student can determine whether a set of issues should be communicated in one message or addressed sequentially based on whether the fixes interact"

learning_objectives:
  - objective: "Provide 2-3 concrete input/output examples that clarify a transformation requirement when a prose description produces inconsistent results"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student writes I/O examples for a date normalization task and observes Claude produce correct output on the first attempt"
  - objective: "Write a test suite first, then iterate by sharing test failures with Claude until all tests pass"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student creates tests for an edge-case-heavy function, shares failures with Claude, and reaches a passing suite within 3 iterations"
  - objective: "Use the interview pattern to have Claude surface design considerations before implementing in an unfamiliar domain"
    proficiency_level: "B1"
    bloom_level: "Analyze"
    assessment_method: "Student prompts Claude to interview them about a feature, then identifies at least two considerations Claude raised that they had not thought of"
  - objective: "Choose between single-message and sequential iteration based on whether fixes interact"
    proficiency_level: "A2"
    bloom_level: "Analyze"
    assessment_method: "Student correctly categorizes 4 scenarios as single-message or sequential and explains the reasoning"

cognitive_load:
  new_concepts: 4
  assessment: "4 concepts (I/O examples as disambiguation, test-driven iteration loop, interview pattern, single-message vs sequential strategy); within B1 limit of 10"

differentiation:
  extension_for_advanced: "Combine all three techniques in a single feature: interview to discover requirements, write I/O examples to lock the spec, generate tests, then iterate to passing. Compare the result to a one-shot implementation of the same feature."
  remedial_for_struggling: "Start with I/O examples only. Skip the interview pattern until comfortable with the test-driven loop. Use the simplest possible transformation (string formatting) before attempting multi-field data conversions."

# Generation metadata
generated_by: "writer-cicd agent"
created: "2026-03-24"
...
teaching_guide:
  ...
  session_group: 3
  session_title: "Iterative Refinement Techniques"
  key_points:
    - "Prose descriptions of transformations are ambiguous; concrete I/O examples eliminate that ambiguity by showing Claude exactly what you mean"
    - "Test-driven iteration inverts the workflow: you define correctness first, then let Claude iterate against failures until it passes"
    - "The interview pattern is most valuable when YOU do not fully understand the domain; Claude surfaces considerations you would have missed"
    - "Single-message iteration is for interacting fixes (changing one breaks another); sequential iteration is for independent fixes"
  misconceptions:
    - "Students think I/O examples are only for simple string transformations; they work for any transformation including JSON reshaping, SQL generation, and code refactoring"
    - "Students assume test-driven iteration means Claude writes the tests too; the power comes from YOU defining expected behavior and Claude implementing to match"
    - "Students confuse the interview pattern with asking Claude for help; the interview pattern specifically has Claude ASK YOU questions, not answer yours"
  discussion_prompts:
    - "When have you described what you wanted in plain English and gotten something completely different? How would I/O examples have helped?"
    - "What is the risk of letting Claude both write the tests AND write the implementation?"
  teaching_tips:
    - "The date normalization example is the strongest demo for I/O examples: 'March 5th 2024' vs '03/05/2024' vs '2024-03-05' shows ambiguity that prose cannot resolve"
    - "For test-driven iteration, have students write deliberately tricky edge cases (empty input, unicode, negative numbers) so Claude has to iterate multiple times"
    - "The interview pattern works best when students pick a domain they genuinely do not know well (tax rules, medical terminology, legal compliance)"
  assessment_quick_check:
    - "Name the three iterative refinement techniques and when each is most useful"
    - "You told Claude 'normalize the dates in this CSV' and got wrong output. What is your next step?"
    - "Your function has 5 bugs. Three of them interact (fixing one changes the behavior of another). Do you report all 5 in one message or fix them sequentially?"
---
...
| Ambiguity                      | What the examples resolve                      |
| :----------------------------- | :--------------------------------------------- |
...
| :----------------------------------------- | :---------------------------- | :--------------------------------------- |
| Fixes touch the same function or data flow | Single message                | One fix changes the context for another  |
| Fixes are in different modules             | Sequential                    | Each fix can be verified independently   |
| You are unsure whether they interact       | Single message                | Safer to let Claude see the full picture |
| There are more than 5 independent fixes    | Sequential, in batches of 2-3 | Keeps each iteration manageable          |
