...
options: [
"AI cannot read or interpret plain text files without structural formatting markers",
"AI will automatically convert your plain text into properly structured markdown format",
"AI will refuse to process any unformatted specification text it encounters entirely",
...
options: [
"Using a different markdown editor with better AI-optimized syntax highlighting capabilities",
"Adding more bold and italic text formatting for visual emphasis throughout the document",
"Using clearer headings to separate distinct features and establish scope boundaries",
"Converting all bullet lists to structured tables for better visual data organization"
],
correctOption: 2,
...
"Each heading text is far too short and needs at least five words"
],
correctOption: 0,
...
options: [
"Avoid using any headings and instead rely on bold text with varied font sizes",
...
options: [
"Avoid headings for approaches and instead use bold text labels for each alternative option",
...
options: [
"Add dependency notes written in parentheses after each endpoint in the list",
"Add bold formatting to highlight endpoints that should be implemented first",
...
{
question: "Your specification mixes ordered and unordered list markers inconsistently: some bullets use dashes (-), some use asterisks (_), some use plus signs (+). How does this affect AI's interpretation?",
options: [
...
explanation: "The `sql tag tells AI this is SQL syntax, setting expectations for language patterns, conventions, and idioms. AI will use SQL-appropriate approaches. However, AI may still adapt syntax based on specific database system or context—the tag provides guidance, not absolute constraints. Tags don't force exact replication (option B is wrong)—they provide language context for informed implementation. Tags aren't just for editors (option C is wrong)—AI uses them to understand language context and appropriate implementation patterns. Tags don't trigger execution (option D is wrong)—code blocks in specifications are examples and templates, not executable commands. The tag helps AI understand: 'interpret this in SQL context, use SQL conventions, and generate SQL-compatible implementation.'",
    ...
    },
    {
      ...
      options: [
        "Error handling always requires separate documentation files, not inline code blocks at all",
        "Code blocks cannot be used for error handling specifications in markdown documents",
        "Use a different language tag to signal error handling intent explicitly to AI",
    ...
    },
    {
      ...
      options: [
        "Use different language tags for buggy versus correct code to distinguish them clearly",
