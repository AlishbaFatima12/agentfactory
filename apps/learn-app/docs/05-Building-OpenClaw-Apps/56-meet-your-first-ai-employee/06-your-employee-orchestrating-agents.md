...
description: "Use the Agent Client Protocol (ACP) to delegate coding tasks to external harnesses like Claude Code, understand session management and thread binding, then verify delegation is real"
keywords:
  [
...
skills:
  - name: "ACP Delegation Setup"
    proficiency_level: "B1"
    category: "Applied"
    bloom_level: "Apply"
    digcomp_area: "Digital Competence"
    measurable_at_this_level: "Student can set up ACP, spawn a coding session with /acp spawn, manage the session lifecycle, and delegate coding tasks to an external harness"
  ...
  - name: "Delegation Verification"
    ...
    measurable_at_this_level: "Student can verify whether an AI agent actually performed delegated work by checking /acp status and inspecting output files"

  - name: "ACP Protocol Understanding"
    proficiency_level: "B1"
    category: "Technical"
    bloom_level: "Apply"
    digcomp_area: "Digital Competence"
    measurable_at_this_level: "Student can spawn an ACP session with an external coding harness, manage session lifecycle with /acp commands, and explain why protocol-based delegation prevents hallucination"

learning_objectives:
  - objective: "Set up ACP and delegate coding tasks to an external harness like Claude Code"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student installs acpx, configures ACP, spawns a persistent session with /acp spawn, and completes a coding task through the harness"

  - objective: "Verify that an AI agent performed work rather than hallucinated it"
    proficiency_level: "B1"
    bloom_level: "Evaluate"
    assessment_method: "Student uses /acp status to confirm a session exists and inspects output files to verify real artifacts were produced"
  ...
  - objective: "Set up ACP delegation and manage sessions using /acp commands"
    proficiency_level: "B1"
    bloom_level: "Apply"
    assessment_method: "Student installs the acpx plugin, spawns a persistent ACP session, checks status, and closes the session using /acp commands"

cognitive_load:
  new_concepts: 4
  concepts_list:
    - "ACP as delegation protocol (managed sessions replacing ad-hoc shell commands)"
    - "Coding harnesses (Claude Code, Codex, Pi, OpenCode, Gemini as interchangeable targets)"
    - "Verification over trust (check /acp status and output files, not claims)"
    - "Two-tier delegation (you manage the employee, ACP manages the harness)"
  assessment: "4 concepts within B1 range (7-10). ACP commands are simple and verifiable. Every concept is grounded in a hands-on exercise with checkable output."

differentiation:
  extension_for_advanced: "Spawn three parallel ACP sessions running different harnesses on different tasks simultaneously. Use /acp sessions to monitor all three and document which finished first and why."
  remedial_for_struggling: "Focus on Exercise 1 only. Run /acp status to confirm the session exists. If it does not, run /acp doctor to check your setup."
...
teaching_guide:
  ...
  misconceptions:
    - "Students think ACP is just a wrapper around one tool — it is a protocol supporting multiple harnesses (Claude, Codex, Pi, OpenCode, Gemini) through the same interface"
    - "Students assume /acp status confirming a session means the work is good — the session ran, but the output still needs human verification"
    - "Students confuse the employee writing code itself with delegating via ACP — the whole point is that the employee is a coordinator, not a coder"
    - "Students think ACP only works with slash commands — on WhatsApp/Telegram, natural language requests trigger the same ACP dispatch layer"
  discussion_prompts:
  ...
  assessment_quick_check:
    ...
    - "Ask: Why does ACP support multiple harnesses instead of just Claude Code? What does harness-agnostic delegation enable?"
---
...
ACP setup is operator-side infrastructure -- you run these commands in your terminal, not through your messaging channel. This is a bootstrap problem: ACP is what _enables_ your employee to delegate, so the employee cannot set up its own delegation protocol. Once ACP is enabled, everything switches to conversational. Setup is yours; usage is your employee's.
:::
...
| `/acp status`                | Show session state, harness, runtime options                               |
...
| `/acp cancel`                | Stop the current in-flight turn                                            |
| `/acp close`                 | End session and unbind threads                                             |
| `/acp sessions`              | List all recent ACP sessions                                               |
| `/acp doctor`                | Backend health check with actionable fixes                                 |
...
| Aspect          | ACP Session                                 | Sub-Agent Run                  |
| --------------- | ------------------------------------------- | ------------------------------ |
| **Runtime**     | External harness (Claude Code, Codex, etc.) | OpenClaw-native agent          |
| **Best for**    | Coding tasks, multi-file work               | Research, analysis, non-coding |
...
| ----------------------------------------------- | -------------------------------------- | ---------------------------------------------------- |
| `ACP runtime backend is not configured`         | Backend plugin missing or disabled     | Run `/acp install` then `/acp doctor`                |
| `ACP is disabled by policy`                     | `acp.enabled=false`                    | `openclaw config set acp.enabled true` + restart     |
| `ACP agent "<id>" is not allowed by policy`     | Harness not in allowlist               | Ask employee: "add claude to ACP allowed agents"     |
| Status check fails or shows no session          | `/acp status` ran against main session | Focus the ACP session first (`/focus <session-key>`) |
| Persistent session refused on WhatsApp/Telegram | Persistent mode on threadless channel  | Use `--mode oneshot` or switch to Discord/TUI        |
...
| --------------------------- | ------------------------------------------------------------------------ |
| Protocol-based delegation   | ACP manages sessions, state, and lifecycle -- no ad-hoc shell commands   |
| Harness-agnostic interface  | Same `/acp spawn` works across Claude, Codex, Pi, OpenCode, and Gemini   |
| Verification over trust     | `/acp status` and output file checks prove work is real                  |
| Channel-aware delegation    | Persistent on Discord/TUI (threads/focus), one-shot on WhatsApp/Telegram |
| Context stays with employee | Research and intent live with your employee, coding goes to the harness  |
