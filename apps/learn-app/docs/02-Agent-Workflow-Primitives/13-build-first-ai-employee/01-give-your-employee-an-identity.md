...
### Prompt 1: Design Your Identity

```
I am building an AI employee for my profession: [your profession].
My specialty area is [specific focus within profession].

Help me design the four sections of my employee's identity file:

1. IDENTITY: Draft a 3-4 sentence introduction covering role, specialty,
   and communication style
2. RULES: Generate 5 "ALWAYS" rules and 5 "NEVER" rules specific to
   [your profession]. Focus on ethical obligations and regulatory requirements.
3. VOCABULARY: List 15 domain terms that a professional in my field uses
   daily but a general AI might not use correctly
4. BOUNDARIES: Identify 5 request types that should be refused or flagged,
   with what the employee should do instead

Be specific to my field. Generic rules like "be helpful" are not useful.
```

**What you are learning:** Domain knowledge externalization. The hardest part of configuring an AI employee is making implicit professional knowledge explicit. You know instinctively what an accountant should never do or what a teacher should always check -- but writing it down forces precision that makes the difference between a useful employee and a liability.

### Prompt 2: Stress-Test Your Boundaries

```
I configured my AI employee with these boundaries:
[paste your Boundaries section]

Now act as an adversarial tester. Generate 5 messages that a real user
might send that would test these boundaries. For each message:
- Explain which boundary it targets
- Predict whether my current rules would catch it
- Suggest how to strengthen the rule if it would slip through

Include at least one message that is subtly out of scope (not obviously
wrong, but something my employee should still flag).
```

**What you are learning:** Boundary robustness testing. In production systems, the edge cases matter more than the obvious cases. A real estate AI that refuses "tell me this house will double in value" is easy. One that also flags "based on recent trends, what would you expect this property to be worth next year?" is robust. This prompt teaches you to think about subtle boundary violations.

### Prompt 3: Evaluate Your Configuration

```
Here is my complete CLAUDE.md for my [profession] AI employee:
[paste your full CLAUDE.md]

Evaluate this configuration as if you were hiring this employee:
1. Would a professional in my field trust this employee's responses?
2. Are there obvious gaps in the rules that could cause problems?
3. Is the vocabulary comprehensive enough for daily professional use?
4. Are the boundaries clear enough that the employee would consistently
   refuse out-of-scope requests?

Score each section 1-5 and explain what would make it a 5.
```

**What you are learning:** Configuration quality assessment. You are learning to evaluate your own work against professional standards -- the same skill you will use every time you deploy an AI system. The gap between "it works" and "I would trust it with real professional tasks" is where quality engineering lives.
