...
Use these prompts to design, build, and evaluate your boundary system.

```
I'm a [your profession]. Help me design a permission boundary system for my AI employee.

List 15 actions I commonly perform. For each one, recommend whether it should be:
- AUTO (employee does it independently)
- REVIEW (employee drafts, I approve before it executes)
- BLOCK (employee never does this)

For each recommendation, explain what specifically goes wrong in my profession
if this action is done incorrectly or without oversight. I need real consequences,
not generic risk statements.
```

**What you're learning:** How to decompose professional work into risk categories. The AI brings breadth (listing actions you might not think of), while you bring depth (knowing which consequences actually matter in your specific context). This is the Evaluate level of Bloom's taxonomy — making judgments based on criteria.

```
Review my boundary table below and stress-test it. For each AUTO action,
describe a scenario where it SHOULD have been REVIEW instead. For each
BLOCK action, describe a scenario where blocking it causes more harm
than allowing it with review.

[Paste your boundary table here]

Then suggest 2-3 edge cases I haven't considered — actions that don't
fit neatly into any category.
```

**What you're learning:** How to pressure-test a permission system before deploying it. Real boundary systems fail at the edges, not the obvious cases. You are teaching the AI your domain constraints while it surfaces scenarios you had not considered — a convergence loop where the final boundary table is stronger than either of you would produce alone.

```
Write the CLAUDE.md rules section that enforces these boundaries for my
NanoClaw employee. The rules should:
1. List each action category with clear instructions
2. For REVIEW actions, specify the exact format for asking my approval
3. Include a fallback rule: "If an action is not listed, treat it as REVIEW"
4. Log all AUTO actions so I can audit them later

Use my boundary table:
[Paste your final boundary table here]
```

**What you're learning:** How to translate a conceptual trust framework into executable instructions. The gap between "I want boundaries" and "boundaries that actually work" is the implementation detail — the exact wording in CLAUDE.md that determines whether your employee stops or proceeds. This is the Create level: building a working system from a design.
