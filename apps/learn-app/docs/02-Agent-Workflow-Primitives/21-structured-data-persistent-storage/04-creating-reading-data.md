...
keywords: ["CRUD", "session", "commit", "rollback", "verification"]
skills:
  - name: "Persistence Verification"
    ...
    measurable_at_this_level: "Student can verify agent-written CRUD operations are safe by reading output and directing failure tests"
  - name: "Session Lifecycle Management"
    proficiency_level: "A2"
    category: "Conceptual"
    bloom_level: "Understand"
    digcomp_area: "Data Literacy"
    measurable_at_this_level: "Student can explain the session lifecycle to understand why agent code needs both a write AND verify step"
learning_objectives:
  - objective: "Verify an agent-written store operation by reading back the result in a separate query"
    proficiency_level: "A2"
    bloom_level: "Apply"
    assessment_method: "Student directs the agent to store one expense and confirms the output shows it was saved"
  - objective: "Direct the agent to test a failure case and confirm rollback leaves zero partial rows"
    proficiency_level: "A2"
    bloom_level: "Apply"
    assessment_method: "Student directs the agent to insert an invalid row and reads the rollback confirmation output"
cognitive_load:
  new_concepts: 4
...
teaching_guide:
  ...
  assessment_quick_check:
    ...
    - "What does it mean that a failed write left zero new rows?"
---
...
Store one expense for Alice in the Food category for $52.50 on January 15, 2024.
Then show me all expenses over $50.
:::
...
:::conversation[What you tell the agent]
Try to store an expense for a user that does not exist (user ID 9999).
Show me what error the database returns.
Then verify zero new rows were created.
:::
