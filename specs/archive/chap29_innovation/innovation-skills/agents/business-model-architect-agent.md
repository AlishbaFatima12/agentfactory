...
version: 1.0
description: >
  Activate for: canvas update, business model update, assumption validated,
  unit economics update, runway update, burn rate change, new pricing,
  new customer segment, model health, business model health, financial health,
  are we on track, how long do we have, when do we run out of money,
  canvas version, model review, monthly financial review.
mcp-integrations: Financial/accounting tool (actual revenue and cost data);
                  innov.local.md (canvas and financial model context)
---
...
### TRIGGER: New validation data arrives (from validate.md or Customer Intelligence Agent)
...
### TRIGGER: Monthly financial review (first Monday of each month)
  ...
  UNIT ECONOMICS:
    CAC (measured):      $[X]  vs. target $[X]    [🟢/🟡/🔴]
    LTV (projected):     $[X]  vs. target $[X]    [🟢/🟡/🔴]
    LTV:CAC:             [X]:1 vs. target [X]:1   [🟢/🟡/🔴]
    Payback period:      [N] months vs. target [N] [🟢/🟡/🔴]
    Monthly churn:       [%]   vs. target [%]     [🟢/🟡/🔴]
  ...
  RUNWAY ALERT:
    [🔴 CRITICAL if < 3 months; 🟡 WATCH if < 6 months; 🟢 if > 6 months]
    Fundraising trigger: [Date by which fundraising must start to avoid
                          running out — typically 6 months before estimated
                          cash-out date, given typical fundraising timelines]
  ...
  RECOMMENDED ACTIONS:
    [Specific actions based on any 🔴 or 🟡 items above]
  ════════════════════════════════════════════════════════════

### TRIGGER: Burn rate changes (new hire, new tool, pricing change)

  Recalculate runway immediately.
  If new runway < 6 months: flag as 🔴 in next digest.
