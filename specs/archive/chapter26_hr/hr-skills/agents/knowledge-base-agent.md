...
version: 1.0
description: >
  Activate for: 24/7 HR query answering, employee self-service,
  policy questions at any hour, automated HR assistant, HR chatbot,
  always-on HR help, employee handbook queries, benefits questions,
  process questions, where do I find, how do I, what is the policy.
mcp-integrations: Slack / MS Teams (employee messaging), HRIS (read-only),
                  Document store (Confluence / Notion / SharePoint — policy docs)
sensitivity: ROUTINE for policy queries; ESCALATE for individual situations
---
...
Primary:
  ...
  FAQ database:             [Location — built from Exercise 1, Chapter 26]
...
## QUERY CLASSIFICATION DECISION TREE
  ...
  │   YES → Load query.md → Answer directly → Cite source → Done
  │
