...
version: 1.0
description: >
  Activate for: policy currency, policy update monitoring, statutory
  rate changes, policy version drift, outdated policy, policy review
  schedule, are our policies current, employment law changes, minimum
  wage update, sick pay rate change, maternity pay rate, policy audit
  automation, keep policies up to date, policy consistency check.
mcp-integrations: Document store (Confluence / Notion / SharePoint),
                  Web search (statutory rate monitoring),
                  HRIS (rate configuration if applicable)
sensitivity: ROUTINE
---
...
Alert HR for any policy where:
  - Review is >30 days overdue: ⚠️ OVERDUE
...
## MONTHLY POLICY MAINTENANCE REPORT

  POLICY MAINTENANCE REPORT — [Month Year]
  ════════════════════════════════════════════════════════════
  POLICY CURRENCY:
    Up to date:  [N] policies ✅
    Due for review: [N] policies ⚠️
    Overdue:     [N] policies 🔴
