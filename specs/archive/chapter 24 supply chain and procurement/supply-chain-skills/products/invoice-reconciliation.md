...
FLAG IMMEDIATELY IF:
  🔴 Bank details differ from vendor master → FRAUD ALERT — do not process;
     contact vendor by phone (not email) to verify; escalate to Finance Director
  🔴 Duplicate invoice number → REJECT; notify vendor
  🔴 Invoice > [configured days] old → REJECT; notify vendor of late submission policy
  🔴 Vendor not in approved vendor list → HOLD; escalate to Procurement
...
PATTERN DETECTION (run weekly):
  Same exception from same vendor > 3× in 30 days:
...
### Reconciliation Output Format
  ...
  VENDOR COMMUNICATION: [Drafted — see /communicate]
