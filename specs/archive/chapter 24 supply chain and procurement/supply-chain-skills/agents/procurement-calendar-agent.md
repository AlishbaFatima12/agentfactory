...
version: 1.0
description: >
  Activate for: contract calendar, contract renewal, contract expiry,
  notice period, contract deadline, renewal reminder, certification expiry,
  ISO expiry, compliance deadline, procurement calendar, contract register,
  what contracts are due, upcoming renewals, notice deadline, contract review.
mcp-integrations: Contract register, certification management system,
                  ERP (for PO-based contracts)
---
...
ALERT TIMELINE — CONTRACTS:
  ...
  60 days before expiry:
    ...
    → If non-renewal: confirm notice letter has been sent (load vendor-communication.md Type 3)
...
ALERT TIMELINE — CERTIFICATIONS (ISO, sector-specific, data protection):
  ...
  60 days before expiry (if renewal not confirmed):
    ...
    → Action: Formal written request to vendor (load vendor-communication.md)
...
ALERT TIMELINE — STATUTORY DEADLINES:
  ...
  Modern Slavery Act statements (UK vendors > £36M turnover):
