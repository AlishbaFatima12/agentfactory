...
version: 1.0
description: >
  Activate for: digest, daily digest, morning brief, morning briefing,
  what's happening today, today's priorities, start of day brief,
  daily brief, what do I need to know today, morning update, daily update,
  what's on my plate today, daily overview, what's due today, week ahead,
  Monday brief, weekly digest, start of week.
plugin-commands: /digest
---
...
The Daily Digest is the agentic morning briefing. It answers the question:
"What do I need to know and do today to keep this week on track?"

A well-designed digest is read in 5 minutes and acted on in 15.
A poorly designed digest becomes another thing to skip.
...
Pull from (in order of priority):
  ...
  2. Google Calendar (via MCP) — today's meetings; upcoming deadlines
  3. Notion / task management (via MCP) — open tasks; overdue items
  4. Gmail (via MCP) — flagged / unread items requiring response
  5. Slack (via MCP) — @mentions; flagged items; direct messages
...
### Digest Output Structure
  ...
  ════════════════════════════════════════════════════════════
  HEADLINE STATUS: [🟢 CLEAR RUNWAY / 🟡 WATCH ITEMS / 🔴 CRITICAL ITEM(S)]

  ── TODAY'S CRITICAL PATH ───────────────────────────────────
  [3–5 items maximum — ordered by importance, not urgency]
  1. [🔴 if blocking/overdue] [Item] — [specific action]
  ...
  ⏳ [Person] — [Task] — due [date] — [AWAITING CONFIRMATION / IN PROGRESS / OVERDUE]
...
### Digest Tone
  ...
  NOT: A list of everything happening (that is the task list)
...
- NEVER omit open delegations — what you are waiting for is as
  important as what you are doing, and it is the most commonly
  forgotten category in manual task management
