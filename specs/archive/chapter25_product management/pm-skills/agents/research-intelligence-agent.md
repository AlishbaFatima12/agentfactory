...
version: 1.0
description: >
  Activate for: research monitoring, user signal monitoring, weekly research
  digest, support ticket trends, NPS trends, feature request trends, user
  feedback digest, emerging user problems, continuous research, user signal
  alert, what are users saying, feedback synthesis automated, track user
  feedback, monitor user sentiment.
mcp-integrations: Zendesk/Intercom (support), Delighted/Typeform (NPS),
                  Canny/ProductBoard (feature requests), App stores (if applicable)
---
